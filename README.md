...
The binary is `klio`. Pipeline: `klio-lexer` → `klio-parser` (→ `klio-ast`) → `klio-interp`, with shared `klio-span` and `klio-diagnostics` crates.

## Layout

```
crates/
  klio-span         Source files, file ids, byte spans, line/col mapping.
  klio-diagnostics  Error/warning model with labels, notes, and rendering.
  klio-lexer        Tokenizer for Kotlin source.
  klio-ast          AST node definitions.
  klio-parser       Recursive-descent parser producing AST + diagnostics.
  klio-interp       Tree-walking interpreter over the AST.
  klio-cli          `klio` binary: lex/parse/run/repl commands.
```
...
```
cargo build
cargo run -p klio-cli -- lex path/to/file.kt
cargo run -p klio-cli -- parse path/to/file.kt
cargo run -p klio-cli -- run path/to/file.kt
cargo run -p klio-cli -- repl
```
...
Near-term scope is the Kotlin language plus a full built-in implementation of the Kotlin stdlib. Loading or interoperating with third-party Kotlin/JVM libraries (Maven artifacts, `.jar`/`.klib` consumption, etc.) is explicitly **out of scope** for the foreseeable future. A program targeting `klio` may only depend on the language and the bundled stdlib.
