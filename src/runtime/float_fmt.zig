...
fn roundToOdd128(g: Pow10, cp: u64) u64 {
    ...
    const y0: u64 = @truncate(y);
    const y1: u64 = @truncate(y >> 64);
    return y1 | @intFromBool(y0 > 1);
}

fn multipleOfPow2(value: u64, e2: i32) bool {
    const sh: u6 = @intCast(e2);
...
fn schubfachF64(bits: u64) DigitsExp {
    ...
    const EXPONENT_BIAS: i32 = 1024 - 1 + (SIGNIFICAND_SIZE - 1); // 1075
    const C_TINY: u64 = 3;

    const ieee_fraction = bits & FRACTION_MASK;
    const ieee_exponent: i32 = @intCast((bits >> 52) & 0x7ff);

    var c: u64 = undefined;
    var q: i32 = undefined;
    var dk: i32 = undefined;
    if (ieee_exponent != 0) {
        dk = 0;
        c = HIDDEN_BIT | ieee_fraction;
        q = ieee_exponent - EXPONENT_BIAS;
        if (0 < -q and -q < SIGNIFICAND_SIZE and multipleOfPow2(c, -q)) {
            const sh: u6 = @intCast(-q);
            return .{ .digits = c >> sh, .exponent = 0 };
        }
    ...
    const lower_boundary_is_closer = ieee_fraction == 0 and ieee_exponent > 1;

    const cbl = 4 * c - 2 + @intFromBool(lower_boundary_is_closer);
    const cb = 4 * c;
    const cbr = 4 * c + 2;

    const k = (q * 1262611 - (if (lower_boundary_is_closer) @as(i32, 524031) else 0)) >> 22;
    const h = q + floorLog2Pow10(-k) + 1;

    const pow10 = pow10F64(-k);
    const hsh: u6 = @intCast(h);
    const vbl = roundToOdd128(pow10, cbl << hsh);
    const vb = roundToOdd128(pow10, cb << hsh);
    const vbr = roundToOdd128(pow10, cbr << hsh);
    ...
    if (s >= 100) {
        const sp10 = (s / 10) * 10;
        const tp10 = sp10 + 10;
        const upin = vbl + out <= sp10 << 2;
        const wpin = (tp10 << 2) + out <= vbr;
        if (upin != wpin) {
            return .{ .digits = if (upin) sp10 else tp10, .exponent = k };
        }
    }

    const t = s + 1;
    const uin = vbl + out <= s << 2;
    const win = (t << 2) + out <= vbr;
    if (uin != win) {
        return .{ .digits = if (uin) s else t, .exponent = k + dk };
    }
...
fn schubfachF32(bits: u32) DigitsExp {
    ...
    const SIGNIFICAND_SIZE: i32 = 24;
    const EXPONENT_BIAS: i32 = 128 - 1 + (SIGNIFICAND_SIZE - 1); // 150
    const C_TINY: u32 = 8;

    const ieee_fraction = bits & FRACTION_MASK;
    const ieee_exponent: i32 = @intCast((bits >> 23) & 0xff);
    ...
    if (ieee_exponent != 0) {
        dk = 0;
        c = @as(u64, HIDDEN_BIT | ieee_fraction);
        q = ieee_exponent - EXPONENT_BIAS;
        if (0 < -q and -q < SIGNIFICAND_SIZE and multipleOfPow2(c, -q)) {
            const sh: u6 = @intCast(-q);
            return .{ .digits = c >> sh, .exponent = 0 };
        ...
        dk = -1;
        c = @as(u64, ieee_fraction * 10);
        q = 1 - EXPONENT_BIAS;
    } else {
        dk = 0;
        c = @as(u64, ieee_fraction);
        q = 1 - EXPONENT_BIAS;
    }
...
fn layoutKotlin(buf: *[MAX_LEN]u8, neg: bool, digits_in: u64, exponent_in: i32) []u8 {
    ...
    const digit_str = std.fmt.bufPrint(&digit_buf, "{d}", .{digits}) catch unreachable;
    const num_digits: i32 = @intCast(digit_str.len);
    const decimal_point = num_digits + exponent;
...
pub const double_to_string = doubleToString;
pub const float_to_string = floatToString;

test "double matches jvm" {
    const a = std.testing.allocator;
    const cases = [_]struct { bits: u64, want: []const u8 }{
        .{ .bits = 1, .want = "4.9E-324" }, // Double.MIN_VALUE
        .{ .bits = 2, .want = "9.9E-324" },
        .{ .bits = 10, .want = "4.9E-323" },
        .{ .bits = 12, .want = "5.9E-323" },
        .{ .bits = 20, .want = "9.9E-323" },
        ...
        .{ .bits = 0x4341C37937E08000, .want = "1.0E16" },
        .{ .bits = 0x7FEFFFFFFFFFFFFF, .want = "1.7976931348623157E308" }, // Double.MAX_VALUE
        .{ .bits = 0x0000000000000000, .want = "0.0" },
        .{ .bits = 0x8000000000000000, .want = "-0.0" },
    };
    for (cases) |c| {
        const got = try doubleToString(a, @bitCast(c.bits));
        defer a.free(got);
    ...
    try expectDouble(a, 1.0e20, "1.0E20");
    try expectDouble(a, 1.0e7, "1.0E7");
    try expectDouble(a, 0.001, "0.001");
    try expectDouble(a, 1.0e-7, "1.0E-7");
    try expectDouble(a, std.math.nan(f64), "NaN");
    try expectDouble(a, std.math.inf(f64), "Infinity");
    try expectDouble(a, -std.math.inf(f64), "-Infinity");
}

fn expectDouble(a: std.mem.Allocator, v: f64, want: []const u8) !void {
    const got = try doubleToString(a, v);
    defer a.free(got);
    try std.testing.expectEqualStrings(want, got);
}

fn expectFloat(a: std.mem.Allocator, v: f32, want: []const u8) !void {
    const got = try floatToString(a, v);
    defer a.free(got);
    try std.testing.expectEqualStrings(want, got);
}

test "float matches jvm" {
    const a = std.testing.allocator;
    const cases = [_]struct { bits: u32, want: []const u8 }{
        .{ .bits = 1, .want = "1.4E-45" }, // Float.MIN_VALUE
        .{ .bits = 2, .want = "2.8E-45" },
        .{ .bits = 3, .want = "4.2E-45" },
        .{ .bits = 71, .want = "9.9E-44" },
        .{ .bits = 0x3F800000, .want = "1.0" },
        .{ .bits = 0x40000000, .want = "2.0" },
        .{ .bits = 0x3F000000, .want = "0.5" },
        .{ .bits = 0x7F7FFFFF, .want = "3.4028235E38" }, // Float.MAX_VALUE
        .{ .bits = 0x80000000, .want = "-0.0" },
    };
    for (cases) |c| {
        const got = try floatToString(a, @bitCast(c.bits));
        defer a.free(got);
        std.testing.expectEqualStrings(c.want, got) catch |e| {
            std.debug.print("bits=0x{x}\n", .{c.bits});
            return e;
    ...
    try expectFloat(a, 3.14159, "3.14159");
    try expectFloat(a, 0.1, "0.1");
    try expectFloat(a, std.math.nan(f32), "NaN");
}

test "double round trips" {
    const a = std.testing.allocator;
    const bits_list = [_]u64{
        1,
        2,
        10,
        0x3FB999999999999A,
    ...
    for (bits_list) |bits| {
        ...
        const s = try doubleToString(a, v);
        defer a.free(s);
        const parsed = try std.fmt.parseFloat(f64, s);
        const parsed_bits: u64 = @bitCast(parsed);
        try std.testing.expectEqual(bits, parsed_bits);
    }
}

test "char unit to string" {
    const a = std.testing.allocator;
    const ascii = try charUnitToString(a, 'A');
    defer a.free(ascii);
    try std.testing.expectEqualStrings("A", ascii);

    const bmp = try charUnitToString(a, 0x00E9); // é
    defer a.free(bmp);
...
test "kotlin aliases match" {
    const a = std.testing.allocator;
    const d = try kotlin_double_to_string(a, 1.0);
    defer a.free(d);
    try std.testing.expectEqualStrings("1.0", d);
    const f = try kotlin_float_to_string(a, 2.0);
    defer a.free(f);
    try std.testing.expectEqualStrings("2.0", f);
}
