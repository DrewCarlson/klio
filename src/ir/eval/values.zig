...
pub fn constMatches(module: *const Module, id: ConstId, v: *const Value) bool {
    ...
    // An unsuffixed integer key takes the subject's type (`when (l: Long) { 42 -> }` compares `42L`), so an
    // integral key compares by value against any signed subject, an unsigned key against any unsigned one.
    switch (c.*) {
        .Int, .Long, .Short, .Byte => {
...
/// Allocator-free `constToValue` for structural comparison; String consts map to `.Null` and are handled in `constMatches`.
