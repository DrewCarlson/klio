//! Static type probes over locals, properties, factories and lateinit markers.
...
/// A local's type derived from its recorded initializer. A call initializer is
/// never written into the local's declared type; it resolves here, on demand, at
/// each use site, so every "is this local typed?" question comes through here.
pub fn localInitTypeRefNamed(b: *FuncBuilder, name: []const u8) Allocator.Error!?ir.TypeRef {
    ...
    // block is bound, the same cycle as the declared-type walk.
    ...
    if (init_expr.* == .Path and init_expr.Path.segments.len == 1) {
        ...
        // its initializer, so the reference is the enclosing class's property, and
        // its declared head types the local as a qualified read would.
        ...
        if (b.resolve(src_name) == null or std.mem.eql(u8, src_name, name)) {
            if (b.ownerClass()) |owner| {
                if (propTypeHeadOn(b, owner, src_name)) |head| {
                    // Two same-simple-name classes make a bare head ambiguous, so
                    // resolve it through the reading file's import graph to the
                    // declaring class's FQN when possible.
    ...
    // initializer's bare calls must not see it: `val iterator = iterator()`.
...
/// applicable to the receiver head, the for-loop protocol's answer for a receiver
/// whose `iterator()` is an extension rather than a member. Disagreeing or
/// generic returns refuse.
pub fn extensionNullaryReturnTypeRef(
    ...
    for (b.module.funcsBySimpleName(mname)) |fid| {
        ...
        if (agreed) |prev| {
            ...
            // Same return on a more specific receiver keeps the more specific
            // declaration, String over CharSequence.
...
/// The trailing lambda's derived return type under concrete input parameter
/// types. Feeds the recorder only, never overload shape selection.
...
/// Re-derive a star in a recorded call-return, where an unbound return-position
/// type parameter was star-erased by the solve, from the call's trailing lambda
/// when the committed candidate's fn-typed last param returns exactly that
/// parameter. Runs after resolution: it improves the recorded local type and
...
pub fn patchStarredCallRecord(
    ...
    // The committed candidate: the receiver's unique lambda-hosting extension of
    // the name and shape.
    ...
    // The starred position in the record must be exactly where the declared return
    // spells that parameter.
...
/// The unique in-scope type-param bound whose class declares property `nm`, the
/// implicit-receiver chase's answer when the splice window's head is a bare param
/// with no bound record.
...
/// The companion object's own class type for a bare class-name reference in value
/// position, or null when the name is shadowed by a value binding or names no
...
pub fn buildStaticReturnArgShapes(
    ...
    for (args, shapes, inferred) |*arg, *shape, *owned| {
        if (shape.ty != null) {
            // A bare class-name Path pre-typed as the class itself is really the
            // companion value, a class name in value position being its companion.
            // The companion's class record carries the supertypes a key-parameter
            // solve projects from.
        ...
        if (arg.* == .MemberRef and !std.mem.eql(u8, runtime.envOnce("KLIO_REFSHAPE") orelse "1", "0")) {
            ...
            // A bare class name in value position is its companion object, whose
            // class extends the `Key` record the solve projects from.
        ...
        if (owned.*) |ty| {
            shape.ty = ty;
            // A call-return derivation whose head is a bare type parameter names
            // the callee's parameter, not the caller's, and judging it against the
            // caller's same-named bound conflates scopes. Advisory only: it types,
            // but never disproves.
            const th = typeHead(std.mem.trimEnd(u8, ty.name, "?"));
...
/// `resolveCall` query, replacing the per-rung `findCand` and `arityMatch` walks.
/// Borrows from `b.allocator`, a lowering scratch arena.
...
/// Argument shapes admitted by an exact or static dispatch proof. The permissive
/// eager type-head fill stays useful for additive runtime ranking, but only
...
/// Builtin value kind a declared parameter type accepts, or null when unknown; an
/// unknown kind never disproves.
...
/// accept the argument kinds, so a bare `Name(args)` constructs the class rather
/// than calling the factory. Argument kinds come from the same evidence the shared
/// scorer sees: a literal, a declared local or param type head, or the typeck
/// type-head channel. Conservative, so only a known-kind-versus-builtin mismatch
/// flips the decision.
...
pub fn shadowedByClass(b: *FuncBuilder, callee: *const Expr, args: []const Expr, arg_names: []const ?[]const u8) Allocator.Error!bool {
    ...
    // A local fn of the name declared in an enclosing body is the nearest scope
    // and shadows any same-named classifier for a bare call.
    if (b.local_fn_overloads.getPtr(name) != null) return false;
    // Resolve the class the same way the construct path below does, through the
    // scope-aware index of file imports, self package, then global, rather than
    // the simple-name-global `classId`, which picks an arbitrary winner on a
    // cross-package collision and would invert the ctor-versus-factory decision.
    ...
    // An abstract, interface, or sealed class cannot be constructed, so a bare
    // `Name(args)` is a same-named factory function. Resolving it as a function
    // finds the factory instead of emitting a `NewInstance` that aborts.
    if (cid.int() < b.module.classes.items.len and b.module.classes.items[cid.int()].is_abstract) return false;
    const nargs = args.len;
    // Overload resolution walks scopes inside-out, so inside the class's own body,
    // its companion included, the constructor is a nearer-scope candidate than any
    // package-level factory and an applicable constructor decides the call.
    if (b.ownerClass()) |oc| {
    // A companion body's owner is the lifted companion class, named with one or
    // more `$Companion` suffixes; stripping them recovers the class whose scope
    // the call sits in.
        ...
        if (in_own_scope and cid.int() < b.module.classes.items.len) {
            ...
            // candidate and wins whenever the arguments fit it, kotlinc resolving
            // the closest scope level with an applicable candidate. Only when the
            // argument types cannot fit the constructor does an applicable
            // same-named function take over, deferred to the runtime choice.
            ...
            // A member of the enclosing class hierarchy is a nearer-scope candidate
            // than a same-named foreign classifier. A class nested in the enclosing
            // chain keeps constructor semantics, its bare name sitting in the
            // member set while a capitalized call to it constructs. Deciding
            // `false` routes through `CallMemberOrGlobal`, whose runtime scoring
            // still reaches the constructor when no member binds.
    ...
    // A captured outer binding of the name is a nearer-scope candidate than an
    // imported classifier. Deciding false routes through the deferred
    // class-carrying form, whose runtime shadow gate lets the captured callable
    // win and still reaches the constructor when nothing binds.
    ...
    // considers an unimported cross-package factory, and without the tier filter
    // pack load order decided whether the builtin ctor won.
    const call_file = callee.Path.segments[0].span.file;
    if (lastArgIsLambda(args)) {
    // A trailing lambda routes to a same-named factory with a function-typed param
    // to receive it; only when none fits is it a ctor.
    ...
    // No lambda: a ctor when the canonical factory cannot take that many args, or
    // no same-named factory is applicable to the positional count.
...
/// Whether a same-named function in scope (tier <= 3) can take these arguments:
/// the arity fits, every named argument names one of its parameters, and its
...
/// Whether the enclosing class declares primary-ctor property `rn` with a declared
/// type that is a bare class type parameter. Such a receiver's member surface is
/// the parameter's bound, and kotlinc commits an in-scope receiver-taking callable
/// of the called name.
fn classPropertyIsBareTp(b: *FuncBuilder, class_name: []const u8, rn: []const u8) ?bool {
    const cid = (if (std.mem.findScalar(u8, class_name, '.') != null)
...
/// As `enclosingCtorPropIsBareTypeParam`, also searching the implicit-receiver
/// tower, which a closure body inside a member-inline splice reaches the owner
/// through.
...
/// Whether class `cid` is declared inside the enclosing class chain, as a nested
/// or inner class of the class being lowered or of one of its outers.
...
/// the hidden delegate binding is reachable, bound in this scope or captured from
/// an enclosing one. Null when `x` is not a mutable delegated local.
/// `lateinitMarkerName` is the hidden binding a local `lateinit var` declares
/// beside its home register; its presence marks a read for the uninitialized
/// check, and a nested lambda sees it through the captured-name set.
...
/// Whether a read of local `name` reads a `lateinit var`. `home` is the register
/// the name resolved to, null when it is only reachable as a capture: the marker
/// must be bound to that same register, so a later same-named plain local or a
/// shadowing lambda parameter reads unchecked.
...
/// Guard a local read with the `lateinit` uninitialized check when the name is a
/// lateinit local; otherwise the value passes through.
...
pub fn lowerDelegateRead(b: *FuncBuilder, name: []const u8) Allocator.Error!?Reg {
    ...
    // An anonymous object's or local class's method closes over the delegate
    // binding like any other enclosing local.
    ...
    // An inner plain binding named like the delegated var outranks the delegate
    // read.
...
pub fn simpleTypeHead(name: []const u8) []const u8 {
    var n = name;
    if (std.mem.findScalar(u8, n, '<')) |lt| n = n[0..lt];
    if (std.mem.findScalarLast(u8, n, '.')) |dot| n = n[dot + 1 ..];
    return n;
}
