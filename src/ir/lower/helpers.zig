...
/// Collect every outer-scope variable name a lambda's body directly
/// mutates (assignment / `++` / `--`). Returns an empty slice for
/// non-lambda args. Output is sorted (mirrors the Rust `BTreeSet`
/// ordering) so the writeback list is deterministic. The caller owns the
/// returned slice.
pub fn lambdaMutatedOuterVars(b: *FuncBuilder, arg: *const Expr) Allocator.Error![][]const u8 {
    ...
    var out = StringSet.init(b.allocator);
    defer out.deinit();
    for (lam.body.stmts) |*s| try mutatedWalkStmt(s, &visible, &out);

    const slice = try b.allocator.alloc([]const u8, out.count());
    var i: usize = 0;
    var it = out.keyIterator();
    while (it.next()) |k| : (i += 1) slice[i] = k.*;
    std.sort.pdq([]const u8, slice, {}, struct {
...
fn mutatedWalkStmt(s: *const Stmt, visible: *const StringSet, out: *StringSet) Allocator.Error!void {
    switch (s.*) {
        .Assign => |a| try mutatedVisitPath(&a.target, visible, out),
        .Expr => |*e| try mutatedWalkExpr(e, visible, out),
        else => {},
    }
}

fn mutatedVisitPath(e: *const Expr, visible: *const StringSet, out: *StringSet) Allocator.Error!void {
    switch (e.*) {
        .Path => |p| {
            if (p.segments.len == 1 and visible.contains(p.segments[0].name)) {
                try out.put(p.segments[0].name, {});
            }
        },
        else => {},
    }
...
fn mutatedWalkExpr(e: *const Expr, visible: *const StringSet, out: *StringSet) Allocator.Error!void {
    switch (e.*) {
        .Postfix => |u| {
            if (u.op == .Inc or u.op == .Dec) try mutatedVisitPath(u.expr, visible, out);
        },
        .Unary => |u| {
            if (u.op == .PreInc or u.op == .PreDec) try mutatedVisitPath(u.expr, visible, out);
        },
        .Block => |b| {
            for (b.stmts) |*s| try mutatedWalkStmt(s, visible, out);
        },
        .If => |f| {
            try mutatedWalkExpr(f.cond, visible, out);
            try mutatedWalkExpr(f.then_branch, visible, out);
            if (f.else_branch) |els| try mutatedWalkExpr(els, visible, out);
        },
        .While => |w| {
            try mutatedWalkExpr(w.cond, visible, out);
            try mutatedWalkExpr(w.body, visible, out);
        },
        .DoWhile => |w| {
            if (w.body) |b| try mutatedWalkExpr(b, visible, out);
            try mutatedWalkExpr(w.cond, visible, out);
        },
        .For => |f| try mutatedWalkExpr(f.body, visible, out),
        .When => |w| {
            for (w.branches) |*br| try mutatedWalkExpr(&br.body, visible, out);
        },
        .Try => |t| {
            for (t.body.stmts) |*s| try mutatedWalkStmt(s, visible, out);
            for (t.catches) |*c| {
                for (c.body.stmts) |*s| try mutatedWalkStmt(s, visible, out);
            }
            if (t.finally) |fb| {
                for (fb.stmts) |*s| try mutatedWalkStmt(s, visible, out);
            }
        },
        else => {},
    }
