//! Builder-side lowering helpers: argument-run materialisation, arg-name and
//! type-arg interning, the AST-to-IR binop mapping, and the closure
...
/// A smart cast narrows an `Any`-typed local to a numeric type for the branch, so
/// comparisons there are the primitive IEEE ones, not the boxed `equals`.
...
/// parameter, whose boxed value makes `==` total-order.
...
/// A read-only or iterable collection type whose `plus`/`minus` returns a `List`
/// regardless of the runtime container, so a `Set` reached through it yields a List.
...
/// True when `e` names a local or param statically typed as a broad collection.
...
/// Whether a bare name reads an enclosing-class property whose declared type head,
/// bounds substituted at registration, is a broad collection. Kotlin resolves
/// `data - x` against the static Iterable.
...
/// return the coerced register so a following `+`/`-` produces a `List`.
...
/// True when `arg` is a lambda whose body assigns to a name the IR's current scope
/// shadows or knows as an outer capture.
...
fn writesWalkExpr(e: *const Expr, visible: *const StringSet) bool {
    return switch (e.*) {
        ...
        // A non-local return from inside the lambda body propagates as
...
/// Resolve the register holding the shared `Value.Cell` for a boxed `var`: in the
/// declaring scope the var's `mutable_home`, once a capturing lambda has loaded it
/// the rebound name, else a capture from the enclosing frame.
pub fn boxedCellReg(b: *FuncBuilder, name: []const u8) Allocator.Error!Reg {
    ...
    // No rebind: a `when` arm lowers into its own block without a scope push, so
    // binding the loaded reg would let a use after the join read a register only the
    // possibly untaken arm defines. A fresh load per use is cheap and correct.
    return c;
}

/// Simple name of a call's callee, used as the implicit label of a lambda literal
/// in its argument list. Null when the callee has no simple name.
...
/// Materialise a contiguous run of argument registers for a Call, `CallMember`,
/// `CallValue` or `NewInstance`. Sub-expressions allocating temporaries otherwise
/// leave non-contiguous numbering, so reserve `args.len` registers up front, lower
/// each arg into scratch, and `Move` it into the matching slot.
pub fn lowerArgRun(b: *FuncBuilder, args: []const Expr) Allocator.Error!struct { Reg, u32 } {
    return lowerArgRunWithArity(b, args, null);
}

/// Emit the coerced constant for an integer or float literal, optionally negated,
/// whose declared parameter type is a numeric primitive differing from the literal's
/// natural type. Only literals are touched, and a negated literal folds.
pub fn coerceNumericLiteralArg(b: *FuncBuilder, e: *const Expr, type_name: []const u8) Allocator.Error!?Reg {
    ...
    switch (e.*) {
        // An integer literal of any kind can re-type to a different numeric
        // primitive parameter, the parsed value carrying the magnitude.
...
pub fn lowerArgRunFull(
    ...
    // Prefer the enclosing call's own name over the ambient pending label: lowering
    // a call-shaped receiver re-arms the ambient state with the receiver's callee.
...
/// As `lowerArgRun`, with an optional per-argument expected lambda arity parallel to
/// `args`: a zero-`->` lambda in slot `j` binds its implicit `it` only when
/// `arg_arity[j] == 1`. Passed explicitly rather than through builder state so a
/// stale value cannot leak into an unrelated call's arguments.
pub fn lowerArgRunWithArity(b: *FuncBuilder, args: []const Expr, arg_arity: ?[]const i16) Allocator.Error!struct { Reg, u32 } {
    ...
    const n = args.len;
    if (n == 0) {
        // Reserve a sentinel slot so the zero-argument reads do not alias an
        // unrelated register.
        return .{ b.allocReg(), 0 };
    }
    const first = b.allocReg();
    ...
    var i: usize = 1;
    while (i < n) : (i += 1) slots[i] = b.allocReg();
    // The call's simple name is the implicit label of any lambda literal directly in
    // this argument list, so re-arm before each argument and clear after. The scoped
    // `current_call_label` wins over the ambient pending label.
    const call_label = b.current_call_label orelse b.pending_lambda_label;
    b.pending_lambda_label = null;
    const arg_broad_masks = b.pending_arg_broad_masks;
    const arg_fn_generic = b.pending_arg_fn_generic;
    ...
    b.pending_arg_fn_generic = null;
    b.pending_arg_lambda_param_types = null;
    // Arguments are not in the call's tail position, so the enclosing expected-type
    // hint must not reach a reified inline call here.
    const prev_expected = b.pushExpected(null);
    for (slots, 0..) |slot, j| {
        // A sibling-solved expected type applies to exactly one arg node.
        if (b.sib_expected_site) |site| {
...
/// Intern an `arg_names` slice into a parallel `[]?ConstId` for `Inst.Call` and
/// friends, empty when every entry is null. The caller owns the slice.
...
/// `internTypeArgs` with the lexical-scope type-rename ladder applied, so a reified
/// type argument naming a mangled nested class records the lifted name.
pub fn internTypeArgsScoped(
    ...
    for (type_args, out) |t, *slot| {
        // A bare name matching an active splice's reified parameter means the bound
        // actual, even when the callee's type params live in another registry.
        ...
        // A nested-generic argument stamps its full spelling so runtime KType
        // materialisation sees the arguments; class-resolving consumers strip at `<`.
        const head = if (t.type_args.len == 0) nm else try renderFullTypeArg(b, nm, &t);
        // Nullability is part of the argument: `filterIsInstance<Int?>()` differs
        // from `filterIsInstance<Int>()` only in the `?`, which the body's `is T` reads.
...
pub fn astBinop(op: AstBinOp) BinOp {
    return switch (op) {
        ...
        .Elvis => .Elvis,
        // `in`, `!in` and assign need dedicated IR instructions, so fall back to Add
        // to keep the lowering pass total; the real forms land elsewhere.
        .In, .NotIn, .Assign => .Add,
    };
}
