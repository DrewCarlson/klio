...
const Server = struct {
    ...
    fn start(self: *Server, io: std.Io) !void {
        ...
        self.io = io;
        self.inner = try addr.listen(io, .{ .reuse_address = true });
        self.port = self.inner.socket.address.getPort();
    ...
    fn serveLoop(self: *Server) void {
        const io = self.io;
        while (true) {
            const stream = self.inner.accept(io) catch {
    ...
    fn handleConn(self: *Server, io: std.Io, stream: net.Stream) void {
        ...
        while (true) {
            ...
            if (head_end) |he| {
                if (data.len >= he + content_len) break;
            }
        }
        const data = ir.buffered();
        const is_post = std.mem.startsWith(u8, data, "POST ");
        const body: []const u8 = blk: {
            if (is_post) {
                if (head_end) |he| break :blk data[he..@min(data.len, he + content_len)];
            }
...
fn runProgram(name: []const u8, src: []const u8, feature: []const u8, expected: []const u8) !void {
    ...
    const r = try runKlio(a, io, &env, &.{ klioBin(&env), "run", "--feature", feature, path });
    if (!r.ok) {
