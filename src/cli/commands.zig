...
fn emitNativeBlock(w: anytype, f: *const ir.Func, st: *const ir.bc.Stream, block: u32, fs: *const ir.bc.FuncStreams) !void {
    ...
    while (pc < code.len) {
        ...
        switch (op) {
            ...
            .const_int => {
                try w.print("  klio_op_const_int(ctx, {d}u, (int32_t)0x{x}u);\n", .{ code[pc + 1], code[pc + 2] });
                pc += 3;
            },
            .move => {
                try w.print("  klio_op_move(ctx, {d}u, {d}u);\n", .{ code[pc + 1], code[pc + 2] });
                pc += 3;
            },
            .load_param => {
            ...
            .cmp_br => {
                try w.print("  switch (klio_op_cmp_br(ctx, {d}u, {d}u, {d}u, {d}u, {d}u, {d}u)) {{\n  case 1: ", .{ block, code[pc + 1], code[pc + 2], code[pc + 3], code[pc + 4], code[pc + 5] });
                try emitEdgeTo(w, fs, code[pc + 6]);
                try w.print("\n  case 0: ", .{});
                try emitEdgeTo(w, fs, code[pc + 7]);
                try w.print("\n  default: return;\n  }}\n", .{});
                closed = true;
                pc += 8;
            },
