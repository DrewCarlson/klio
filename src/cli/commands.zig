...
fn emitNativeBlock(w: anytype, f: *const ir.Func, st: *const ir.bc.Stream, block: u32, fs: *const ir.bc.FuncStreams) !void {
    ...
    while (pc < code.len) {
        ...
        switch (op) {
            ...
            .move => {
                try w.print("  if (KV.usable) memcpy(kv_slot(regs, {d}u), kv_slot(regs, {d}u), KV.value_size); else klio_op_move(ctx, {d}u, {d}u);\n", .{ code[pc + 1], code[pc + 2], code[pc + 1], code[pc + 2] });
                pc += 3;
            },
            .load_param => {
...
fn emitBinSite(w: anytype, block: u32, inst_idx: u32, kind_raw: u32, dst: u32, lhs: u32, rhs: u32) !void {
    ...
    try w.print("      if (tl == KV.tag_int && tr == KV.tag_int) {{\n", .{});
    ...
    if (boxed_eq) {
        try w.print("      }} else if (tl == KV.tag_long && tr == KV.tag_long) {{\n", .{});
    } else {
        try w.print("      }} else if ((tl == KV.tag_int || tl == KV.tag_long) && (tr == KV.tag_int || tr == KV.tag_long)) {{\n", .{});
    }
    try w.print("        int64_t a = (tl == KV.tag_int) ? (int64_t)kv_int(bl) : kv_long(bl);\n", .{});
    try w.print("        int64_t b = (tr == KV.tag_int) ? (int64_t)kv_int(br_) : kv_long(br_); (void)a; (void)b;\n", .{});
...
fn emitCmpBrSite(w: anytype, fs: *const ir.bc.FuncStreams, block: u32, inst_idx: u32, kind_raw: u32, dst: u32, lhs: u32, rhs: u32, t_block: u32, f_block: u32) !void {
    ...
    if (hot != null and hot.?.is_bool) {
        ...
        try w.print("    if (KV.usable) {{\n      uint64_t tl = kv_tag(bl), tr = kv_tag(br_);\n", .{});
        if (boxed_eq) {
            try w.print("      int have = (tl == KV.tag_int && tr == KV.tag_int) || (tl == KV.tag_long && tr == KV.tag_long);\n", .{});
        } else {
            try w.print("      int have = (tl == KV.tag_int || tl == KV.tag_long) && (tr == KV.tag_int || tr == KV.tag_long);\n", .{});
        }
        try w.print("      if (have) {{\n", .{});
        try w.print("        int64_t a = (tl == KV.tag_int) ? (int64_t)kv_int(bl) : kv_long(bl);\n", .{});
