...
//! evaluator's own arm bodies (`ir.eval.nativeOp*`), so emitted code shares
...
/// after the performance profile is chosen, since that choice of reclaim mode
/// is what decides `usable`.
...
/// The generated file's emit-time copy of the layout, frozen as constants in its
/// inline fast paths. Verified against the live fill: a .c linked against a
/// runtime with a different layout gets the whole hot view disabled and the
/// exported per-op entries carry it, rather than reading through wrong offsets.
...
fn fillHotLayoutSlot() void {
    ...
    if (hot_layout_slot) |slot| {
        ...
        // The generated file's frozen constants must match the live layout
        // exactly or every inline fast path reads garbage, so a mismatch
        // disables the whole view and the exported helpers carry it.
...
fn runFileBody(path: [:0]const u8) c_int {
    ...
    fillHotLayoutSlot();
    // The same backing the `klio` binary installs for the resolved profile: by
    // default the collector over the page-returning slab.
...
/// Run the Kotlin program at `path` exactly as `klio run <path>` would, on the
/// default process-lifetime arena profile. Returns the process exit code, 0 for
/// success and 1 for diagnostics or a runtime error.
///
/// The work runs on an explicit large stack switched to in place on the calling
/// thread: a transpiled binary's C `main` runs on the libc crt with whatever the
/// process rlimit gives, typically 8MB, and lowering a deeply-nested expression
/// recurses past that. Staying on the caller's thread keeps the emitted `main`
/// on the process main thread, where a program that opens a window must run.
...
/// Run the Kotlin program at `path` against the pre-baked dependency base at
/// `base_image`, exactly as `klio run-image` would. Transpiled programs use this
/// entry: the emitted ids are meaningful only against the module assembled from
/// that exact artifact. Same large-stack switch as `klio_rt_run_file`.
...
/// neither parses nor lowers, the boot a bundle gets. The emitted ids are
/// meaningful against exactly this artifact.
...
/// The hot-view layout descriptor: byte offsets into `runtime.Value` measured
/// against the same build the library runs, so the generated C's inline scalar
/// ops are correct by construction rather than by a frozen contract. `usable` is
/// false when the process runs a reclaim mode whose register writes must release
/// the old value; the emitted code then falls back to the exported helpers.
...
export fn klio_rt_hot_layout(out: *HotLayout) void {
    ...
    // per-thread reclaim off unless the process requested a reclaim mode
    // (KLIO_RECLAIM), and the live flag is not set on this thread when the slot
    // fills, so the request decides. The object view also honors KLIO_OBJVIEW=0.
...
/// Register a transpiled function for `fid`; the interpreter's frame loop runs
/// it in place of the bytecode stream. Call before `klio_rt_run_file`, since the
/// table is read-only once the program runs. `fqn` guards the fid: an entry whose
/// name does not match the function lowered to that fid is ignored.
...
/// Declare the table sizes of the module the emitter walked. A frame whose
/// module disagrees runs interpreted: the emitted const and func ids index
/// these tables and mean nothing against any other module.
...
// The native object ABI: what a compiled program calls. Compiled code is the
// program, with no module to look a class up in, so a class arrives as an
// emitted descriptor and is built here at startup; from then on a field is
// addressed by the index resolved when the C was written, never by name.
//
// Instances are ordinary `InstanceData` cells, so a compiled object flows into
// runtime collections, prints through the runtime's renderer, and is traced by
// the collector exactly as an interpreted one is.

/// A `Value` as C sees it: two words, no tag union. Compiled code never reads
/// the inside, it hands them back to the entry points below. This shape and its
/// conversions live in the runtime so the coroutine driver, which resumes native
/// continuations through the same form, shares one definition.
...
// Roots: the collector is precisely rooted and never scans the native stack, so
// a compiled frame publishes its object slots here. Scalars stay in C locals and
// are not published, since nothing on the heap depends on them.
...
/// installs its view of compiled frames. Without it a compiled program never
/// collects: the GC is armed by the `klio_rt_run_*` entries, which a compiled
/// program never calls.
...
/// collectable. Without it every allocation is minted permanent and the heap
/// only grows.
export fn klio_nat_begin() void {
    runtime.gc.alloc_perm = false;
}

/// The safe point, polled at loop back edges, where an allocating loop would
/// otherwise run to the end of the heap. Every slot the collector may follow is
/// in a published frame at that moment, and nothing holds a borrow across it.
...
/// Print a value the way the interpreter prints it. Compiled code uses this for
/// anything but a plain scalar, so rendering never drifts.
...
// Lists: collection operations are data-structure work on the runtime's own
// types, so a compiled program performs them directly rather than through any
// dispatch. The emitter recognizes the stdlib entry points by name.
...
// Arrays: a primitive array is a packed scalar buffer, so an `IntArray` holds
// int32 elements rather than boxed values. The emitter knows the element kind
// statically and names it here, which keeps an indexed read a load rather than
// an unbox.
...
// and answers either its result or COROUTINE_SUSPENDED. When it suspends it
// pushes its continuation, the emitted resume function plus that frame, onto the
// suspension the interpreter's coroutine driver already parks. Everything above
// that is shared: the scheduler, the virtual clock, the Job graph, `delay`,
// `runBlocking`.

// The stdlib, called directly: every stdlib operation the interpreter performs
// is a named entry in one table, and a compiled program calls the same entries
// rather than growing its own copy of `List`, `Map`, `Set` and `String`. The
// emitter names the declaration and the runtime looks it up and runs it. The one
// thing the caller must supply is how to invoke a closure, since a higher-order
// operation like `forEach` calls back.
...
/// argument, as for `klio_nat_stdlib`.
...
/// interpreter's own (the scheduler, the virtual clock, the Job graph, the park
/// and resume order) and asks its host for exactly two things: start a queued
/// block, and resume a parked continuation. In a compiled program both are
/// native calls, so the whole driver is shared rather than written twice.
const NativeCoroHost = struct {
    ...
    pub fn resumeRaw(
        ...
        // evaluator instantiated against a stub host.
...
// The remaining scalar kinds: `Char` prints as a character and `Short`/`Byte`
// render as themselves, so they cannot ride in an `Int`; the box carries kind.
...
// Capture cells: a `var` a lambda captures becomes a shared box, since the
// lambda and the enclosing function must see each other's writes.
...
/// anywhere, since any block carrying one is refused, so a throw always leaves
/// the program and is reported the way an uncaught exception is.
export fn klio_nat_throw(v: CValue) noreturn {
    ...
    // throw the way the same program reports it interpreted.
...
/// not shapes the emitter lays out, so a compiled program builds one here rather
/// than as an instance with fields. `type_id` is the type's preorder number in
/// the program's throwable hierarchy, which is what a handler tests against.
...
/// comparisons: `catch (e: AppError)` sees every type under it, however deep.
