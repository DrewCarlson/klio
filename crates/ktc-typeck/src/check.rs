...
use ktc_resolver::Resolution;
use ktc_span::Span;
use ktc_types::{builtin_by_name, convert_type_ref_lossy, Type};
...
struct ClassInfo {
    ...
    decl_file: Option<ktc_span::FileId>,
}
...
struct Checker<'a> {
    ...
    fn_visibility: HashMap<String, Vec<(Visibility, ktc_span::FileId)>>,
    /// Visibility + declaring file for each top-level property.
    prop_visibility: HashMap<String, (Visibility, ktc_span::FileId)>,
    ...
    fn_annotations: HashMap<String, Vec<Vec<ktc_ast::Annotation>>>,
    /// Annotations of each top-level property by simple name.
    prop_annotations: HashMap<String, Vec<ktc_ast::Annotation>>,
}
...
impl<'a> Checker<'a> {
    ...
    fn check_m28_class(&mut self, c: &Class) {
        ...
        for tp in &c.type_params {
            if matches!(tp.variance, ktc_ast::Variance::Invariant) {
                continue;
            }
            for m in &c.members {
                if let Decl::Function(f) = m {
...
fn has_unsafe_variance(anns: &[ktc_ast::Annotation]) -> bool {
    anns.iter().any(|a| {
        a.path
            .last()
...
fn has_published_api(anns: &[ktc_ast::Annotation]) -> bool {
    anns.iter().any(|a| {
        a.path
            .last()
...
impl<'r> Checker<'r> {
    ...
    fn compute_expr_ty(&mut self, expr: &Expr, expected: Option<&Type>) -> Type {
        match expr {
            Expr::IntLit { kind, .. } => {
                ...
                if matches!(kind, ktc_ast::IntLitKind::Long) {
                    return Type::Long;
                }
            ...
            Expr::FloatLit { kind, .. } => {
                if matches!(kind, ktc_ast::FloatLitKind::Float) {
                    return Type::Float;
                }
                if let Some(t) = expected {
    ...
    fn check_published_api_use(
        ...
        target_anns: &[ktc_ast::Annotation],
        use_span: Span,
    ) {
    ...
    fn check_lambda(&mut self, params: &[ktc_ast::Ident], body: &Block, expected: Option<&Type>) -> Type {
...
mod tests {
    ...
    fn check_src(src: &str) -> TypeCheck {
        ...
        let r = ktc_resolver::resolve(&ast);
        typecheck(&ast, &r)
    }
