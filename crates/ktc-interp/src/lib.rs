...
//! `ktc-runtime` and are shared with `ktc-stdlib`. The interpreter walks
...
struct TailrecFrame {
    name: String,
    sites: Rc<std::collections::HashSet<ktc_span::Span>>,
}

#[derive(Clone)]
struct ExtensionFn {
    decl: Rc<ktc_ast::Function>,
    env: Rc<RefCell<Env>>,
}

#[derive(Clone)]
struct ExtensionProp {
    decl: Rc<ktc_ast::Property>,
    env: Rc<RefCell<Env>>,
}
...
impl Interpreter {
    ...
    pub fn new() -> Self {
        ...
        for (name, fqn) in IMPLICIT_ALIASES {
            if let Some(func) = ktc_stdlib::implementation(fqn) {
    ...
    fn push_reified_frame(&mut self, decl: &ktc_ast::Function, type_args: &[ktc_ast::TypeRef]) {
        if !decl.is_inline {
            return;
        }
    ...
    fn pop_reified_frame(&mut self, decl: &ktc_ast::Function) {
        if !decl.is_inline {
            return;
        }
    ...
    fn eval_expr(
        ...
        match expr {
            Expr::IntLit { value, kind, .. } => match kind {
                ktc_ast::IntLitKind::Long => Ok(Value::Long(*value)),
                ktc_ast::IntLitKind::Int => Ok(Value::new_int(*value as i32)),
            },
            Expr::BoolLit { value, .. } => Ok(Value::Bool(*value)),
            Expr::NullLit { .. } => Ok(Value::Null),
            Expr::FloatLit { value, kind, .. } => match kind {
                ktc_ast::FloatLitKind::Float => Ok(Value::Float(*value as f32)),
                ktc_ast::FloatLitKind::Double => Ok(Value::Double(*value)),
            },
            ...
            Expr::For { vars, iter, body, .. } => {
                ...
                let items: Box<dyn Iterator<Item = Value>> = match it {
                    Value::Range { start, end, step, kind } => {
                        ...
                        match kind {
                            ktc_runtime::RangeKind::Long => Box::new(it.map(Value::Long)),
                            ktc_runtime::RangeKind::Int => {
            ...
            Expr::Index { receiver, args, .. } => {
                ...
                let Some(func) = ktc_stdlib::implementation(&fqn) else {
            ...
            Expr::AnonFun { params, body, span, .. } => {
                let block = match body.as_deref() {
                    ...
                    Some(FunctionBody::Expr(e)) => ktc_ast::Block {
                        stmts: vec![ktc_ast::Stmt::Expr(ktc_ast::Expr::Return {
                    ...
                    None => ktc_ast::Block { stmts: Vec::new(), span: *span },
                };
                Ok(Value::Lambda {
    ...
    fn eval_object_expr(
        ...
        let synth = ktc_ast::Class {
            name: ktc_ast::Ident { name: anon_name, span },
            type_params: Vec::new(),
            ...
            visibility: ktc_ast::Visibility::Public,
            annotations: Vec::new(),
            span,
        };
    ...
    fn eval_when(
        &mut self,
        subject: Option<&Expr>,
        subject_binding: Option<&ktc_ast::WhenBinding>,
        branches: &[ktc_ast::WhenBranch],
        _span: ktc_span::Span,
        env: &Rc<RefCell<Env>>,
        out: &mut dyn Output,
    ) -> Result<Value, RuntimeError> {
        ...
        for branch in branches {
            ...
            for p in &branch.patterns {
                match &p.kind {
                    ...
                    ktc_ast::WhenPatternKind::IsType(ty) => {
                        let Some(s) = &subject_val else {
                            return Err(RuntimeError::Type(
                    ...
                    ktc_ast::WhenPatternKind::NotIsType(ty) => {
                        let Some(s) = &subject_val else {
                            return Err(RuntimeError::Type(
                                "`!is` pattern requires a when subject".into(),
                            ));
    ...
    fn try_eval_array_constructor(
        ...
        use ktc_runtime::PrimitiveArrayKind;
    ...
    fn eval_generate_sequence(
        ...
        use ktc_runtime::{SequenceData, SequenceSource};
    ...
    fn try_eval_sequence_member(
        ...
        use ktc_runtime::{SeqOp, SequenceData};
    ...
    fn materialize_sequence(
        &mut self,
        data: &ktc_runtime::SequenceData,
        out: &mut dyn Output,
    ) -> Result<Vec<Value>, RuntimeError> {
        use ktc_runtime::{SeqOp, SequenceData, SequenceSource};
    ...
    fn apply_sort_op(
        &mut self,
        op: &ktc_runtime::SeqOp,
        items: &mut Vec<Value>,
        out: &mut dyn Output,
    ) -> Result<(), RuntimeError> {
        use ktc_runtime::SeqOp;
        match op {
    ...
    fn pull_sequence_source(
        &mut self,
        source: &ktc_runtime::SequenceSource,
        state: &mut SourceState,
        out: &mut dyn Output,
    ) -> Result<Option<Value>, RuntimeError> {
        use ktc_runtime::SequenceSource;
        match (source, state) {
                ...
                let inner: Vec<Value> = match r {
                    ...
                    Value::Range { start, end, step, kind } => {
                        ...
                        let mk = |c: i64| match kind {
                            ktc_runtime::RangeKind::Long => Value::Long(c),
                            ktc_runtime::RangeKind::Int => Value::new_int(c as i32),
                        };
                        if step > 0 {
                            while cur <= end {
                                v.push(mk(cur));
    ...
    fn try_eval_collection_higher_order(
        ...
        let initial = if matches!(name, "fold") {
            ...
            Expr::NullLit { span: ktc_span::Span::new(ktc_span::FileId(0), 0, 0) }
        };
                        ...
                        Value::Range { start, end, step, kind } => {
                            for n in range_iter(start, end, step) {
                                out_items.push(match kind {
                                    ktc_runtime::RangeKind::Long => Value::Long(n),
                                    ktc_runtime::RangeKind::Int => Value::new_int(n as i32),
                                });
                            }
                        }
                        other => {
    ...
    fn build_class_shell(
        ...
        let mut nested: Vec<(String, Rc<ktc_ast::Class>)> = Vec::new();
        for m in &c.members {
        ...
        let parent_ctor_args: Vec<Rc<ktc_ast::Expr>> = c
            .supertype_args
            .iter()
            .find_map(|a| a.as_ref())
            .map(|args| args.iter().map(|e| Rc::new(e.clone())).collect())
            .unwrap_or_default();
        let secondary_ctors: Vec<Rc<ktc_ast::SecondaryCtor>> = c
            .secondary_ctors
            .iter()
        ...
        let mut supertype_delegates: Vec<ktc_runtime::SupertypeDelegate> = Vec::new();
        for (idx, opt) in c.supertype_delegates.iter().enumerate() {
            ...
            let iface_name = c
                ...
                .unwrap_or_default();
            supertype_delegates.push(ktc_runtime::SupertypeDelegate {
    ...
    fn run_secondary_ctor(
        ...
        match delegation {
            ktc_ast::CtorDelegation::This(dargs) => {
            ...
            ktc_ast::CtorDelegation::Super(dargs) => {
    ...
    fn call_builtin_delegate_get(
        &mut self,
        d: &Rc<RefCell<ktc_runtime::DelegateKind>>,
        prop: &Value,
        env: &Rc<RefCell<Env>>,
        out: &mut dyn Output,
    ) -> Result<Value, RuntimeError> {
        ...
        match snapshot {
            ktc_runtime::DelegateKind::Lazy { producer, cached } => {
                if let Some(v) = cached {
                    return Ok(v);
                }
                let v = self.call_value_no_args(&producer, env, out)?;
                *d.borrow_mut() = ktc_runtime::DelegateKind::Lazy {
                    producer,
                    cached: Some(v.clone()),
                };
                Ok(v)
            }
            ktc_runtime::DelegateKind::Observable { value, .. } => Ok(value),
            ktc_runtime::DelegateKind::NotNull { value, .. } => match value {
                Some(v) => Ok(v),
                None => {
    ...
    fn call_builtin_delegate_set(
        ...
        let snapshot = d.borrow().clone();
        match snapshot {
            ktc_runtime::DelegateKind::Lazy { .. } => Err(RuntimeError::Type(
                "cannot assign to a `val` property delegated by `lazy`".into(),
            )),
            ktc_runtime::DelegateKind::Observable { value: old, on_change } => {
                *d.borrow_mut() = ktc_runtime::DelegateKind::Observable {
            ...
            ktc_runtime::DelegateKind::NotNull { name, .. } => {
                *d.borrow_mut() = ktc_runtime::DelegateKind::NotNull {
        ...
        match member {
            "observable" => {
                ...
                Ok(Value::Delegate(Rc::new(RefCell::new(
                    ktc_runtime::DelegateKind::Observable {
                        value: initial,
                        on_change,
                    },
                ))))
            ...
            "notNull" => {
                ...
                Ok(Value::Delegate(Rc::new(RefCell::new(
                    ktc_runtime::DelegateKind::NotNull {
                        value: None,
                        name: String::new(),
                    },
                ))))
        ...
        if let Value::Array { items, .. } = &receiver {
            match name {
                ...
                "indices" => {
                    ...
                    return Ok(Value::Range { start: 0, end: n - 1, step: 1, kind: ktc_runtime::RangeKind::Int });
                }
                _ => {}
            }
        }
    ...
    fn eval_call(
        ...
        if let Some(name) = simple_callee_name(callee) {
            if name == "lazy" && args.len() == 1 {
                ...
                return Ok(Value::Delegate(Rc::new(RefCell::new(
                    ktc_runtime::DelegateKind::Lazy { producer, cached: None },
                ))));
            }
        }
        ...
        if let Expr::Member { receiver, name, safe, .. } = callee {
            ...
            if let Some(func) = ktc_stdlib::implementation(&type_fqn) {
...
const LATEINIT_SENTINEL_FQN: &str = "__ktc_lateinit_uninitialized__";
...
impl OpState {
    fn for_op(op: &ktc_runtime::SeqOp) -> Self {
        use ktc_runtime::SeqOp;
        match op {
...
fn reorder_intrinsic_args(
    ...
    let Some(params) = ktc_stdlib::param_names(fqn) else {
        return Ok(args);
    };
...
fn is_sort_op(op: &ktc_runtime::SeqOp) -> bool {
    use ktc_runtime::SeqOp;
...
impl SourceState {
    fn new(source: &ktc_runtime::SequenceSource) -> Self {
        match source {
            ktc_runtime::SequenceSource::Items(_) => Self::Items { index: 0 },
            ktc_runtime::SequenceSource::Generate { .. } => Self::Generate {
                last: None,
                started: false,
            },
        }
...
fn choose_extreme(name: &str, a: Value, b: Value) -> Result<Value, RuntimeError> {
    let cmp = match (&a, &b) {
        ...
        (Value::Double(x), Value::Int(y)) => x
            ...
            .unwrap_or(std::cmp::Ordering::Equal),
        (Value::String(x), Value::String(y)) => ktc_stdlib::compare_utf16(x, y),
...
pub(crate) fn collect_tail_self_calls(
    body: &ktc_ast::FunctionBody,
    fn_name: &str,
) -> std::collections::HashSet<ktc_span::Span> {
    let mut sites = std::collections::HashSet::new();
    match body {
        ktc_ast::FunctionBody::Block(b) => walk_block_tail(b, true, fn_name, &mut sites),
        ktc_ast::FunctionBody::Expr(e) => walk_expr_tail(e, true, fn_name, &mut sites),
    }
    sites
}
...
fn walk_block_tail(
    ...
    tail: bool,
    fn_name: &str,
    sites: &mut std::collections::HashSet<ktc_span::Span>,
) {
    let n = b.stmts.len();
    for (i, s) in b.stmts.iter().enumerate() {
        ...
        match s {
            ktc_ast::Stmt::Expr(e) => walk_expr_tail(e, stmt_tail, fn_name, sites),
            ktc_ast::Stmt::Decl(_) => {}
            ktc_ast::Stmt::Assign { target, value, .. } => {
                walk_expr_tail(target, false, fn_name, sites);
                walk_expr_tail(value, false, fn_name, sites);
            }
            ktc_ast::Stmt::DestructuringDecl { init, .. } => {
    ...
    match e {
        ...
        Expr::When { subject, branches, .. } => {
            ...
            for b in branches {
                for p in &b.patterns {
                    match &p.kind {
                        ktc_ast::WhenPatternKind::Value(ex)
                        | ktc_ast::WhenPatternKind::InRange(ex)
                        | ktc_ast::WhenPatternKind::NotInRange(ex) => {
...
fn eval_binop(op: BinOp, l: Value, r: Value) -> Result<Value, RuntimeError> {
    ...
    match (op, &l, &r) {
        (Lt, String(a), String(b)) => Ok(Bool(ktc_stdlib::compare_utf16(a, b).is_lt())),
        (Le, String(a), String(b)) => Ok(Bool(ktc_stdlib::compare_utf16(a, b).is_le())),
        (Gt, String(a), String(b)) => Ok(Bool(ktc_stdlib::compare_utf16(a, b).is_gt())),
        (Ge, String(a), String(b)) => Ok(Bool(ktc_stdlib::compare_utf16(a, b).is_ge())),
