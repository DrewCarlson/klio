...
fn apply_binop(op: BinOp, l: &Value, r: &Value) -> Result<Value, EvalError> {
    ...
    match (op, l, r) {
        ...
        // Unsigned comparisons — same-type, by the unsigned value
        // (Kotlin's `UInt`/`ULong`/`UShort`/`UByte` compareTo).
        (BinOp::Less, Value::UInt(a), Value::UInt(b)) => Ok(Bool(a < b)),
        (BinOp::LessEq, Value::UInt(a), Value::UInt(b)) => Ok(Bool(a <= b)),
        (BinOp::Greater, Value::UInt(a), Value::UInt(b)) => Ok(Bool(a > b)),
        (BinOp::GreaterEq, Value::UInt(a), Value::UInt(b)) => Ok(Bool(a >= b)),
        (BinOp::Less, Value::ULong(a), Value::ULong(b)) => Ok(Bool(a < b)),
        (BinOp::LessEq, Value::ULong(a), Value::ULong(b)) => Ok(Bool(a <= b)),
        (BinOp::Greater, Value::ULong(a), Value::ULong(b)) => Ok(Bool(a > b)),
        (BinOp::GreaterEq, Value::ULong(a), Value::ULong(b)) => Ok(Bool(a >= b)),
        (BinOp::Less, Value::UShort(a), Value::UShort(b)) => Ok(Bool(a < b)),
        (BinOp::LessEq, Value::UShort(a), Value::UShort(b)) => Ok(Bool(a <= b)),
        (BinOp::Greater, Value::UShort(a), Value::UShort(b)) => Ok(Bool(a > b)),
        (BinOp::GreaterEq, Value::UShort(a), Value::UShort(b)) => Ok(Bool(a >= b)),
        (BinOp::Less, Value::UByte(a), Value::UByte(b)) => Ok(Bool(a < b)),
        (BinOp::LessEq, Value::UByte(a), Value::UByte(b)) => Ok(Bool(a <= b)),
