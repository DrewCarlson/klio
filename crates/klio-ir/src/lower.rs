...
/// True when the fn's body contains a non-local return — any
/// nested lambda body that does a bare `return` (returning from
/// `f` itself, not the lambda's own scope). Tree walker handles
/// this via `RuntimeError::Return` caught at the function
/// boundary; the IR doesn't model it directly, so we route the
/// whole body through EvalAst when present.
fn body_has_non_local_return(f: &klio_ast::Function) -> bool {
    fn walk_block(b: &klio_ast::Block) -> bool {
        b.stmts.iter().any(walk_stmt)
    }
    fn walk_stmt(s: &klio_ast::Stmt) -> bool {
        match s {
            klio_ast::Stmt::Expr(e) => walk_expr(e),
            klio_ast::Stmt::Assign { value, .. } => walk_expr(value),
            klio_ast::Stmt::Decl(klio_ast::Decl::Property(p)) => {
                p.init.as_ref().map_or(false, walk_expr)
            }
            _ => false,
        }
    }
    fn walk_expr(e: &Expr) -> bool {
        match e {
            Expr::Lambda { body, .. } => {
                // A `return` inside a lambda body is non-local.
                body.stmts.iter().any(lambda_returns)
            }
            Expr::Call { args, .. } => args.iter().any(walk_expr),
            Expr::Block(blk) => walk_block(blk),
            Expr::If { cond, then_branch, else_branch, .. } => {
                walk_expr(cond)
                    || walk_expr(then_branch)
                    || else_branch.as_ref().map_or(false, |e| walk_expr(e))
            }
            Expr::While { cond, body, .. } => walk_expr(cond) || walk_expr(body),
            Expr::DoWhile { body, cond, .. } => {
                walk_expr(cond) || body.as_ref().map_or(false, |b| walk_expr(b))
            }
            Expr::For { body, .. } => walk_expr(body),
            Expr::Member { receiver, .. } => walk_expr(receiver),
            Expr::Index { receiver, args, .. } => {
                walk_expr(receiver) || args.iter().any(walk_expr)
            }
            Expr::Binary { lhs, rhs, .. } => walk_expr(lhs) || walk_expr(rhs),
            Expr::Unary { expr, .. } => walk_expr(expr),
            Expr::Postfix { expr, .. } => walk_expr(expr),
            Expr::StringTemplate { parts, .. } => parts.iter().any(|p| match p {
                klio_ast::StringPart::Interp(e) => walk_expr(e),
                _ => false,
            }),
            _ => false,
        }
    }
    fn lambda_returns(s: &klio_ast::Stmt) -> bool {
        match s {
            klio_ast::Stmt::Expr(e) => expr_contains_return(e),
            klio_ast::Stmt::Assign { value, .. } => expr_contains_return(value),
            klio_ast::Stmt::Decl(klio_ast::Decl::Property(p)) => {
                p.init.as_ref().map_or(false, expr_contains_return)
            }
            _ => false,
        }
    }
    fn expr_contains_return(e: &Expr) -> bool {
        match e {
            Expr::Return { .. } => true,
            Expr::Lambda { .. } => false, // nested lambdas have their own return scope
            Expr::Block(b) => b.stmts.iter().any(lambda_returns),
            Expr::If { cond, then_branch, else_branch, .. } => {
                expr_contains_return(cond)
                    || expr_contains_return(then_branch)
                    || else_branch.as_ref().map_or(false, |e| expr_contains_return(e))
            }
            Expr::Call { callee, args, .. } => {
                expr_contains_return(callee) || args.iter().any(expr_contains_return)
            }
            Expr::Member { receiver, .. } => expr_contains_return(receiver),
            Expr::Binary { lhs, rhs, .. } => expr_contains_return(lhs) || expr_contains_return(rhs),
            Expr::Unary { expr, .. } | Expr::Postfix { expr, .. } => expr_contains_return(expr),
            _ => false,
        }
    }
    match &f.body {
        Some(klio_ast::FunctionBody::Block(blk)) => walk_block(blk),
        Some(klio_ast::FunctionBody::Expr(e)) => walk_expr(e),
        None => false,
    }
}
...
fn lower_function_body(module: &mut crate::Module, f: &klio_ast::Function) -> crate::Func {
    ...
    // Fast path for non-local return: when any nested lambda body
    // contains a bare `return` (i.e. returning from the enclosing
    // fn), drop the IR lowering for the body entirely and route
    // through tree walker via EvalAst — tree walker's call_lambda
    // throws RuntimeError::Return which call_function catches at
    // the fn boundary.
    if body_has_non_local_return(f) {
        if let Some(klio_ast::FunctionBody::Block(blk)) = &f.body {
            let dst = b.alloc_reg();
            // Param names are visible inside the AST eval.
            ...
            let captures: Vec<Reg> = captured_names
                ...
                .collect();
            // Wrap the block in a synthetic Block expression so we
            // can hand it to EvalAst's `ast: Box<Expr>`.
            let block_expr = Expr::Block(blk.clone());
            b.push(Inst::EvalAst {
                dst,
                ast: Box::new(block_expr),
                captured_names,
                captures,
            });
            b.terminate(Terminator::Return(Some(dst)));
            let mut func = b.finish(f.name.name.clone(), f.name.name.clone(), crate::TypeRef::unit());
            func.params = f
                .params
                .iter()
                .map(|p| crate::Param {
