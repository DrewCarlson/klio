...
//! `Float.MIN_VALUE` → `1.4E-45`) where Rust's std float formatting,
//! which optimizes purely for shortest *length*, renders a different
//! (also round-tripping) string. The layout step (scientific-vs-plain
//! threshold, `E` notation, trailing `.0`) follows Kotlin's
//! `Double.toString` contract.
//!
//! Ported from the reference C++ (abolz/Drachennest, Boost-1.0) via the
...
fn schubfach_f64(bits: u64) -> (u64, i32) {
    ...
    let ieee_fraction = bits & FRACTION_MASK;
    ...
    let k = (q * 1262611 - if lower_boundary_is_closer { 524031 } else { 0 }) >> 22;
    let h = q + floor_log2_pow10(-k) + 1;

    let pow10 = pow10_f64(-k);
    let vbl = round_to_odd_128(pow10, cbl << h);
    let vb = round_to_odd_128(pow10, cb << h);
    let vbr = round_to_odd_128(pow10, cbr << h);
...
fn layout_kotlin(neg: bool, mut digits: u64, mut exponent: i32) -> String {
    ...
    if scientific {
        ...
        out.push_str(itoa_signed(sci_exp).as_str());
        ...
        out.push_str(&digit_str[0..dp]);
        out.push('.');
        out.push_str(&digit_str[dp..]);
    } else {
        out.push_str(&digit_str);
        for _ in 0..(decimal_point - num_digits) {
            out.push('0');
        }
        out.push_str(".0");
    }
...
#[inline]
fn itoa_signed(x: i32) -> String {
    x.to_string()
}

/// `kotlin.Double.toString` — see module docs.
#[must_use]
pub fn double_to_string(d: f64) -> String {
    ...
    layout_kotlin(neg, digits, exponent)
}
...
mod tests {
    ...
    /// Reference strings are `java.lang.Double.toString` / `Float.toString`
    /// (Zulu/OpenJDK 21). The boundary subnormals are where Rust's
    /// shortest-length formatting diverged from the JVM before this port.
    #[test]
    fn double_matches_jvm() {
        let cases: &[(u64, &str)] = &[
            ...
            (0x7FEFFFFFFFFFFFFF, "1.7976931348623157E308"), // Double.MAX_VALUE
        ...
        assert_eq!(double_to_string(1.0e-7), "1.0E-7");
    ...
    fn float_matches_jvm() {
        let cases: &[(u32, &str)] = &[
            ...
            (0x40000000, "2.0"),
            (0x3F000000, "0.5"),
            (0x7F7FFFFF, "3.4028235E38"), // Float.MAX_VALUE
            (0x80000000, "-0.0"),
        ];
        for &(bits, want) in cases {
            assert_eq!(
    ...
    /// Every finite `f64` formats to a string that parses back to the same
    /// bits — the round-trip invariant the shortest representation must hold.
    #[test]
    fn double_round_trips() {
        for bits in [
            ...
            assert_eq!(s.parse::<f64>().unwrap().to_bits(), bits, "{s}");
        }
    }
}
