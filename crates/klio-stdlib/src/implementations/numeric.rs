use super::{
    Arc, CallCtx, RuntimeError, Value, arg2, kotlin_float_total_cmp, make_exception, num_extreme,
};
...
pub(crate) fn recv_int_radix(v: Option<&Value>, what: &str) -> Result<i64, RuntimeError> {
    match v {
        ...
        Some(v) => v
            .as_i64()
            .ok_or_else(|| RuntimeError::Type(format!("{what} radix must be Int"))),
    }
}

// radix is validated to 2..=36 before the cast
...
pub(crate) fn int_to_string(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    ...
    Ok(Value::String(Arc::new(int_to_radix_string(
        n,
        radix as u32,
    ))))
}

// non-negative branch reinterprets the magnitude; digit fits the radix
#[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
...
// Kotlin Int.toDouble() widens; precision loss only at very large Long inputs
...
// Kotlin Long.toFloat() loses precision
#[allow(clippy::cast_precision_loss)]
...
// Kotlin UInt/ULong.toInt() reinterprets the bit pattern
...
// Kotlin UInt/ULong.toLong() reinterprets the bit pattern
#[allow(clippy::cast_possible_wrap)]
...
// Kotlin toShort() reinterprets the low 16 bits as signed
#[allow(clippy::cast_possible_wrap)]
...
// Kotlin toByte() reinterprets the low 8 bits as signed
#[allow(clippy::cast_possible_wrap)]
...
// Kotlin unsigned.toDouble() loses precision past 2^53
#[allow(clippy::cast_precision_loss)]
...
// Kotlin unsigned.toFloat() loses precision
#[allow(clippy::cast_precision_loss)]
...
pub(crate) fn float_to_double(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    Ok(Value::Double(f64::from(recv_float(
        ctx.args,
        "Float.toDouble",
    )?)))
}
...
pub(crate) fn float_to_string(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    ...
    Ok(Value::String(Arc::new(
        klio_runtime::kotlin_float_to_string(d),
    )))
}
...
pub(crate) fn float_is_infinite(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    Ok(Value::Bool(
        recv_float(ctx.args, "Float.isInfinite")?.is_infinite(),
    ))
}
pub(crate) fn float_is_finite(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    Ok(Value::Bool(
        recv_float(ctx.args, "Float.isFinite")?.is_finite(),
    ))
}
pub(crate) fn float_compare_to(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    ...
    let b = ctx
        ...
        .ok_or_else(|| RuntimeError::Type("Float.compareTo requires a number".into()))?;
    // `compareTo` is a total order (NaN greatest, -0.0 < 0.0), unlike the
    // IEEE `<`/`>` operators.
    Ok(Value::new_int(
        kotlin_float_total_cmp(f64::from(a), f64::from(b)) as i64,
    ))
}

// Double additional conversions (Float).
// Kotlin Double.toFloat() truncates precision
#[allow(clippy::cast_possible_truncation)]
...
pub(crate) fn double_to_short(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    Ok(Value::new_short(f64_to_i32_kotlin(recv_double(
        ctx.args,
        "Double.toShort",
    )?)))
}
pub(crate) fn double_to_byte(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    Ok(Value::new_byte(f64_to_i32_kotlin(recv_double(
        ctx.args,
        "Double.toByte",
    )?)))
}

// Int operands truncate to 32 bits, matching Kotlin Int arithmetic
#[allow(clippy::cast_possible_truncation)]
...
// shift amount is masked to 0..31, so the unsigned cast cannot lose data
...
// Kotlin Int.ushr() reinterprets bits; shift amount masked to 0..31
#[allow(clippy::cast_sign_loss, clippy::cast_possible_wrap)]
pub(crate) fn int_ushr(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    int_binop(ctx, "Int.ushr", |a, b| {
        ((a as u32).wrapping_shr((b & 31) as u32)) as i32
    })
}
...
pub(crate) fn long_binop<F: Fn(i64, i64) -> i64>(
    ...
    let Some(b) = ctx.args.get(1).and_then(Value::as_i64) else {
        return Err(RuntimeError::Type(format!(
            "{what} requires a Long argument"
        )));
    };
    Ok(Value::Long(op(a, b)))
}
pub(crate) fn long_and(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    long_binop(ctx, "Long.and", |a, b| a & b)
}
pub(crate) fn long_or(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    long_binop(ctx, "Long.or", |a, b| a | b)
}
pub(crate) fn long_xor(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    long_binop(ctx, "Long.xor", |a, b| a ^ b)
}
pub(crate) fn long_inv(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    Ok(Value::Long(!recv_int(ctx.args, "Long.inv")?))
}
// shift amount is masked to 0..63, so the unsigned cast cannot lose data
#[allow(clippy::cast_sign_loss)]
...
// Kotlin Long.ushr() reinterprets bits; shift amount masked to 0..63
#[allow(clippy::cast_sign_loss, clippy::cast_possible_wrap)]
pub(crate) fn long_ushr(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    long_binop(ctx, "Long.ushr", |a, b| {
...
// in-range value truncates toward zero, matching Kotlin Double.toInt()
#[allow(clippy::cast_possible_truncation)]
...
// bounds widen exactly enough to saturate; in-range value truncates toward zero
...
// Kotlin Double.toRawBits() reinterprets the 64-bit pattern as Long
#[allow(clippy::cast_possible_wrap)]
...
// Kotlin Double.toBits() reinterprets the 64-bit pattern as Long
#[allow(clippy::cast_possible_wrap)]
...
// Kotlin Double.fromBits() reinterprets the Long bit pattern
#[allow(clippy::cast_sign_loss)]
...
// Kotlin Float.toRawBits() reinterprets the 32-bit pattern as Int
#[allow(clippy::cast_possible_wrap)]
...
// Kotlin Float.toBits() reinterprets the 32-bit pattern as Int
#[allow(clippy::cast_possible_wrap)]
...
// Kotlin Float.fromBits() reinterprets the Int bit pattern
#[allow(clippy::cast_sign_loss)]
...
pub(crate) fn double_is_infinite(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    Ok(Value::Bool(
        recv_double(ctx.args, "Double.isInfinite")?.is_infinite(),
    ))
}
pub(crate) fn double_is_finite(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    Ok(Value::Bool(
        recv_double(ctx.args, "Double.isFinite")?.is_finite(),
    ))
}
pub(crate) fn double_compare_to(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    ...
    let b = ctx
        ...
        .get(1)
        .and_then(Value::as_f64)
        .ok_or_else(|| RuntimeError::Type("Double.compareTo requires a number".into()))?;
    // `compareTo` is a total order (NaN greatest, -0.0 < 0.0), unlike the
    // IEEE `<`/`>` operators.
...
pub(crate) fn bool_to_string(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    let Some(Value::Bool(b)) = ctx.args.first() else {
        return Err(RuntimeError::Type(
            "Boolean.toString requires a Boolean".into(),
        ));
    };
...
pub(crate) fn int_coerce_in(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    ...
    match &ctx.args[1..] {
        [Value::Range { start, end, .. }] => Ok(Value::new_int(v.max(*start).min(*end))),
...
// reinterprets each integer width unsigned; the bit count fits Int
#[allow(clippy::cast_sign_loss, clippy::cast_possible_wrap)]
...
// Kotlin Int.toChar() keeps the low 16 bits as the UTF-16 code unit
#[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
