...
//! `ktc-lexer`. Expression grammar is implemented as a Pratt parser with
...
use ktc_diagnostics::{Diagnostic, DiagnosticSink};
use ktc_lexer::{Keyword, Token, TokenKind};
use ktc_span::{FileId, Span};
...
impl<'src, 'tok> Parser<'src, 'tok> {
    ...
    fn parse_int_literal(&mut self, base: ktc_lexer::NumBase) -> Expr {
        let tok = self.bump();
        let raw_text = self.text(tok.span);
        let (digits, radix): (&str, u32) = match base {
            ktc_lexer::NumBase::Decimal => (raw_text.trim_end_matches(['L', 'u', 'U']), 10),
            ktc_lexer::NumBase::Hex => (
                raw_text.trim_start_matches("0x").trim_start_matches("0X")
                    .trim_end_matches(['L', 'u', 'U']),
                16,
            ),
            ktc_lexer::NumBase::Binary => (
        ...
        let kind = if has_l_suffix {
            ktc_ast::IntLitKind::Long
        } else {
            ktc_ast::IntLitKind::Int
        };
    ...
    fn parse_float_literal(&mut self) -> Expr {
        ...
        let kind = if has_f_suffix {
            ktc_ast::FloatLitKind::Float
        } else {
            ktc_ast::FloatLitKind::Double
        };
...
mod tests {
    ...
    fn parse(src: &str) -> (ktc_ast::KotlinFile, DiagnosticSink) {
    ...
    fn parses_hello_world() {
        ...
        assert!(matches!(file.decls[0], ktc_ast::Decl::Function(ref f) if f.name.name == "main"));
    }
    ...
    fn expression_body_function() {
        ...
        assert!(!diags.has_errors());
        let ktc_ast::Decl::Function(f) = &file.decls[0] else { panic!() };
        assert!(matches!(f.body, Some(ktc_ast::FunctionBody::Expr(_))));
    }
    ...
    fn pratt_precedence() {
        ...
        let ktc_ast::Decl::Function(f) = &file.decls[0] else { panic!() };
        let Some(ktc_ast::FunctionBody::Block(b)) = &f.body else { panic!() };
        let ktc_ast::Stmt::Decl(ktc_ast::Decl::Property(p)) = &b.stmts[0] else { panic!() };
        let init = p.init.as_ref().unwrap();
        let ktc_ast::Expr::Binary { op: outer, rhs, .. } = init else { panic!() };
        assert_eq!(*outer, ktc_ast::BinOp::Add);
        assert!(matches!(**rhs, ktc_ast::Expr::Binary { op: ktc_ast::BinOp::Mul, .. }));
    }
    ...
    fn assignment_is_a_statement() {
        ...
        let ktc_ast::Decl::Function(f) = &file.decls[0] else { panic!() };
        let Some(ktc_ast::FunctionBody::Block(b)) = &f.body else { panic!() };
        assert!(matches!(b.stmts.last().unwrap(), ktc_ast::Stmt::Assign { .. }));
    }
    ...
    fn compound_assignment_recognized() {
        ...
        let ktc_ast::Decl::Function(f) = &file.decls[0] else { panic!() };
        let Some(ktc_ast::FunctionBody::Block(b)) = &f.body else { panic!() };
        let ktc_ast::Stmt::Assign { op, .. } = b.stmts.last().unwrap() else { panic!() };
        assert_eq!(*op, ktc_ast::AssignOp::Add);
    }
    ...
    fn string_template_parts() {
        ...
        let Some(ktc_ast::FunctionBody::Block(b)) = &f.body else { panic!() };
        let ktc_ast::Stmt::Decl(ktc_ast::Decl::Property(p)) = &b.stmts[0] else { panic!() };
        let ktc_ast::Expr::StringTemplate { parts, .. } = p.init.as_ref().unwrap() else { panic!() };
        assert_eq!(parts.len(), 4);
        assert!(matches!(parts[0], ktc_ast::StringPart::Text(_)));
        assert!(matches!(parts[1], ktc_ast::StringPart::ShortInterp(_)));
        assert!(matches!(parts[2], ktc_ast::StringPart::Text(_)));
        assert!(matches!(parts[3], ktc_ast::StringPart::Interp(_)));
    }
    ...
    fn for_loop_parses() {
        ...
        let ktc_ast::Decl::Function(f) = &file.decls[0] else { panic!() };
        let Some(ktc_ast::FunctionBody::Block(b)) = &f.body else { panic!() };
        assert!(matches!(b.stmts[0], ktc_ast::Stmt::Expr(ktc_ast::Expr::For { .. })));
    }
    ...
    fn member_chain_and_safe_call() {
        ...
        let Some(ktc_ast::FunctionBody::Block(b)) = &f.body else { panic!() };
        let ktc_ast::Stmt::Decl(ktc_ast::Decl::Property(p)) = &b.stmts[0] else { panic!() };
        let ktc_ast::Expr::Member { safe, .. } = p.init.as_ref().unwrap() else { panic!() };
        assert!(*safe);
    }
    ...
    fn property_type<'a>(file: &'a ktc_ast::KotlinFile) -> &'a ktc_ast::TypeRef {
        let ktc_ast::Decl::Property(p) = &file.decls[0] else { panic!("expected property") };
    ...
    fn function_type_multi_param() {
        ...
        assert!(!diags.has_errors(), "{:?}", diags.diagnostics());
        let ktc_ast::Decl::Function(fn_) = &file.decls[0] else { panic!() };
    ...
    fn recovers_from_top_level_garbage() {
        ...
        assert!(file.decls.iter().any(|d| matches!(d, ktc_ast::Decl::Function(f) if f.name.name == "main")));
    }
    ...
    fn visibility_modifiers_round_trip() {
        ...
        for d in &file.decls {
            match d {
                ktc_ast::Decl::Function(f) => vis.push((f.name.name.as_str(), f.visibility)),
                ktc_ast::Decl::Class(c) => vis.push((c.name.name.as_str(), c.visibility)),
                ktc_ast::Decl::Property(p) => vis.push((p.name.name.as_str(), p.visibility)),
                _ => {}
            }
        }
        assert_eq!(
            vis,
            vec![
                ("a", ktc_ast::Visibility::Private),
                ("B", ktc_ast::Visibility::Internal),
                ("c", ktc_ast::Visibility::Protected),
                ("d", ktc_ast::Visibility::Public),
            ]
        );
    }
    ...
    fn visibility_defaults_to_public() {
        ...
        assert!(!diags.has_errors());
        let ktc_ast::Decl::Function(f) = &file.decls[0] else { panic!() };
        assert_eq!(f.visibility, ktc_ast::Visibility::Public);
    }
    ...
    fn annotation_use_site_target() {
        ...
        assert!(!diags.has_errors(), "{:?}", diags.diagnostics());
        let ktc_ast::Decl::Class(c) = &file.decls[0] else { panic!() };
        let cp = &c.primary_params[0];
        assert_eq!(cp.annotations.len(), 1);
        assert_eq!(cp.annotations[0].use_site, Some(ktc_ast::AnnotationUseSite::Field));
    ...
    fn annotation_array_form() {
        ...
        assert!(!diags.has_errors(), "{:?}", diags.diagnostics());
        let ktc_ast::Decl::Property(p) = &file.decls[0] else { panic!() };
        assert_eq!(p.annotations.len(), 2);
        assert!(p.annotations.iter().all(|a| a.use_site == Some(ktc_ast::AnnotationUseSite::Field)));
    ...
    fn when_subject_binding_parsed() {
        ...
        assert!(!diags.has_errors(), "{:?}", diags.diagnostics());
        let ktc_ast::Decl::Function(f) = &file.decls[0] else { panic!() };
        let Some(ktc_ast::FunctionBody::Expr(e)) = &f.body else { panic!() };
        let ktc_ast::Expr::When { subject, subject_binding, .. } = e else { panic!() };
    ...
    fn as_basic() {
        ...
        let ktc_ast::Decl::Function(f) = &file.decls[0] else { panic!() };
        let Some(ktc_ast::FunctionBody::Expr(e)) = &f.body else { panic!() };
        let ktc_ast::Expr::As { ty, safe, .. } = e else { panic!("got {e:?}") };
        assert!(!*safe);
    ...
    fn as_safe() {
        ...
        let ktc_ast::Decl::Function(f) = &file.decls[0] else { panic!() };
        let Some(ktc_ast::FunctionBody::Expr(e)) = &f.body else { panic!() };
        let ktc_ast::Expr::As { safe, ty, .. } = e else { panic!() };
        assert!(*safe);
        assert_eq!(ty.name.name, "String");
    }
    ...
    fn as_chains_left_associative() {
        ...
        let ktc_ast::Decl::Function(f) = &file.decls[0] else { panic!() };
        let Some(ktc_ast::FunctionBody::Expr(e)) = &f.body else { panic!() };
        let ktc_ast::Expr::As { expr: outer_expr, ty: outer_ty, .. } = e else { panic!() };
        assert_eq!(outer_ty.name.name, "B");
        let ktc_ast::Expr::As { ty: inner_ty, .. } = outer_expr.as_ref() else { panic!() };
    ...
    fn anon_fun_expr_body() {
        ...
        let ktc_ast::Stmt::Decl(ktc_ast::Decl::Property(p)) = &b.stmts[0] else { panic!() };
        let init = p.init.as_ref().unwrap();
        let ktc_ast::Expr::AnonFun { params, return_ty, body, .. } = init else { panic!() };
        assert_eq!(params.len(), 1);
        assert_eq!(return_ty.as_ref().unwrap().name.name, "Int");
        assert!(matches!(body.as_deref(), Some(ktc_ast::FunctionBody::Expr(_))));
    }
    ...
    fn anon_fun_block_body() {
        ...
        let Some(ktc_ast::FunctionBody::Block(b)) = &f.body else { panic!() };
        let ktc_ast::Stmt::Decl(ktc_ast::Decl::Property(p)) = &b.stmts[0] else { panic!() };
        let ktc_ast::Expr::AnonFun { body, .. } = p.init.as_ref().unwrap() else { panic!() };
        assert!(matches!(body.as_deref(), Some(ktc_ast::FunctionBody::Block(_))));
    }
    ...
    fn anon_fun_optional_param_types() {
        ...
        let Some(ktc_ast::FunctionBody::Block(b)) = &f.body else { panic!() };
        let ktc_ast::Stmt::Decl(ktc_ast::Decl::Property(p)) = &b.stmts[0] else { panic!() };
        let ktc_ast::Expr::AnonFun { params, .. } = p.init.as_ref().unwrap() else { panic!() };
    ...
    fn anon_fun_with_receiver() {
        ...
        let Some(ktc_ast::FunctionBody::Block(b)) = &f.body else { panic!() };
        let ktc_ast::Stmt::Decl(ktc_ast::Decl::Property(p)) = &b.stmts[0] else { panic!() };
        let ktc_ast::Expr::AnonFun { receiver_ty, .. } = p.init.as_ref().unwrap() else { panic!() };
    ...
    fn body_stmts(f: &ktc_ast::Function) -> &[ktc_ast::Stmt] {
        match f.body.as_ref().unwrap() {
            ktc_ast::FunctionBody::Block(b) => &b.stmts,
            ktc_ast::FunctionBody::Expr(_) => panic!("not block-bodied"),
        }
    }
    ...
    fn infix_call_user_defined() {
        ...
        assert!(!diags.has_errors(), "{:?}", diags.diagnostics());
        let ktc_ast::Decl::Function(f0) = &file.decls[0] else { panic!() };
        assert!(f0.is_infix);
        let ktc_ast::Decl::Function(main) = &file.decls[1] else { panic!() };
        let stmts = body_stmts(main);
        let ktc_ast::Stmt::Decl(ktc_ast::Decl::Property(p)) = &stmts[0] else { panic!() };
        let init = p.init.as_ref().unwrap();
    ...
    fn infix_call_no_newline_break() {
        ...
        let ktc_ast::Decl::Function(main) = &file.decls[0] else { panic!() };
        let stmts = body_stmts(main);
        assert_eq!(stmts.len(), 2);
        let ktc_ast::Stmt::Decl(_) = &stmts[0] else { panic!() };
        let ktc_ast::Stmt::Expr(Expr::Call { is_infix, .. }) = &stmts[1] else { panic!() };
        assert!(!*is_infix);
    }
    ...
    fn return_with_label() {
        ...
        let ktc_ast::Decl::Function(main) = &file.decls[0] else { panic!() };
        let stmts = body_stmts(main);
        let ktc_ast::Stmt::Expr(Expr::Labeled { label, .. }) = &stmts[0] else { panic!() };
    ...
    fn labeled_loop() {
        ...
        let ktc_ast::Decl::Function(main) = &file.decls[0] else { panic!() };
        let stmts = body_stmts(main);
        let ktc_ast::Stmt::Expr(Expr::Labeled { label, expr, .. }) = &stmts[0] else { panic!() };
    ...
    fn typealias_top_level() {
        ...
        assert!(!diags.has_errors(), "{:?}", diags.diagnostics());
        let ktc_ast::Decl::TypeAlias(a) = &file.decls[0] else { panic!() };
    ...
    fn extension_property_on_user_class() {
        ...
        assert!(!diags.has_errors(), "{:?}", diags.diagnostics());
        let ktc_ast::Decl::Property(p) = &file.decls[1] else { panic!() };
    ...
    fn typealias_in_class_body_parses() {
        ...
        assert!(matches!(&c.members[0], ktc_ast::Decl::TypeAlias(_)));
    }
    ...
    fn delegation_supertype_parsed() {
        ...
        assert!(!diags.has_errors(), "{:?}", diags.diagnostics());
        let ktc_ast::Decl::Class(c) = &file.decls[1] else { panic!() };
        assert_eq!(c.supertypes.len(), 1);
        assert_eq!(c.supertype_delegates.len(), 1);
        assert!(c.supertype_args[0].is_none());
        assert!(matches!(&c.supertype_delegates[0], Some(ktc_ast::Expr::Path { .. })));
    }
    ...
    fn delegation_with_class_body_not_consumed_as_lambda() {
        ...
        let ktc_ast::Decl::Class(c) = &file.decls[1] else { panic!() };
        assert_eq!(c.members.len(), 1);
        assert!(matches!(&c.members[0], ktc_ast::Decl::Function(f) if f.name.name == "extra"));
    }
    ...
    fn data_object_parsed() {
        ...
        assert!(!diags.has_errors(), "{:?}", diags.diagnostics());
        let ktc_ast::Decl::Object(o) = &file.decls[0] else { panic!() };
        assert!(o.is_data);
    ...
    fn spread_arg_parsed() {
        ...
        assert!(!diags.has_errors(), "{:?}", diags.diagnostics());
        let ktc_ast::Decl::Function(main) = &file.decls[1] else { panic!() };
        let Some(ktc_ast::FunctionBody::Block(b)) = &main.body else { panic!() };
        // The second statement is the call show(*a).
        let ktc_ast::Stmt::Expr(ktc_ast::Expr::Call { args, .. }) = &b.stmts[1] else {
            panic!("expected call");
        };
        assert!(matches!(&args[0], ktc_ast::Expr::Spread { .. }));
    }
}
