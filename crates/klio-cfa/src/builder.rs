...
use crate::ir::{
    BasicBlock, BlockId, Cfg, Edge, EdgeKind, LabelId, LoopId, Node, Reg, Terminator,
};
use klio_span::Span;
...
impl CfgBuilder {
    ...
    pub fn set_terminator(&mut self, blk: BlockId, term: Terminator) {
        ...
        let succs: Vec<Edge> = match &term {
            Terminator::Goto(t) => vec![Edge { block: *t, kind: EdgeKind::Normal }],
            Terminator::Branch { then_blk, else_blk, .. } => vec![
                Edge { block: *then_blk, kind: EdgeKind::True },
                Edge { block: *else_blk, kind: EdgeKind::False },
            ],
            Terminator::Switch { arms, default, .. } => {
                let mut v: Vec<Edge> = arms
                    .iter()
                    .map(|a| Edge { block: a.target, kind: EdgeKind::Normal })
                    .collect();
                v.push(Edge { block: *default, kind: EdgeKind::Normal });
                v
            }
        ...
        for edge in &succs {
            self.blocks[edge.block.0 as usize]
                .preds
                .push(Edge { block: blk, kind: edge.kind.clone() });
        }
    ...
    pub fn add_edge(&mut self, from: BlockId, to: BlockId, kind: EdgeKind) {
        self.blocks[from.0 as usize]
            .succs
            .push(Edge { block: to, kind: kind.clone() });
    ...
    fn unwire_succs(&mut self, blk: BlockId) {
        ...
        for edge in prior {
            ...
            if let Some(idx) = dst.iter().position(|e| e.block == blk && e.kind == edge.kind) {
                dst.swap_remove(idx);
            }
        }
    }
