...
use klio_cfa::{
    CfgBuilder, ExprRef, FieldId, Node, Pattern, Place, SwitchArm, Symbol, Terminator,
    print_cfg,
};
...
fn span(s: u32, e: u32) -> Span {
    Span { file: FileId(0), start: s, end: e }
}
...
fn straight_line() {
    ...
    let r0 = b.new_reg();
    let r1 = b.new_reg();
    b.push(entry, Node::Eval { reg: r0, expr: ExprRef { span: span(0, 1), ty: Type::Int } });
    b.push(entry, Node::Eval { reg: r1, expr: ExprRef { span: span(2, 3), ty: Type::Int } });
    b.push(
        entry,
        Node::Assign { lhs: Place::Local(Symbol("tmp".into())), rhs: r0, span: span(4, 7) },
    );
...
fn branch_join() {
    ...
    b.push(entry, Node::Eval { reg: cond, expr: ExprRef { span: span(0, 4), ty: Type::Boolean } });
    b.set_terminator(entry, Terminator::Branch { cond, then_blk, else_blk });

    b.push(then_blk, Node::Assume { reg: cond, polarity: true });
    b.push(then_blk, Node::Eval { reg: rt, expr: ExprRef { span: span(5, 6), ty: Type::Int } });
    b.set_terminator(then_blk, Terminator::Goto(join));

    b.push(else_blk, Node::Assume { reg: cond, polarity: false });
    b.push(else_blk, Node::Eval { reg: rf, expr: ExprRef { span: span(7, 8), ty: Type::Int } });
...
fn is_check_arm_carries_assume_is() {
    ...
    let subj = b.new_reg();
    b.push(entry, Node::Eval { reg: subj, expr: ExprRef { span: span(0, 1), ty: Type::Any } });
    b.set_terminator(
        entry,
        Terminator::Switch {
            reg: subj,
            arms: vec![SwitchArm {
                pattern: Pattern::Is { ty: Type::String, polarity: true },
                target: str_arm,
            }],
            default: def_arm,
        },
...
fn loop_with_backedge() {
    ...
    let cond = b.new_reg();
    b.push(head, Node::Eval { reg: cond, expr: ExprRef { span: span(0, 1), ty: Type::Boolean } });
    b.set_terminator(head, Terminator::Branch { cond, then_blk: body, else_blk: exit });

    b.push(body, Node::Assume { reg: cond, polarity: true });
    b.push(body, Node::Backedge { loop_id: lid });
    b.set_terminator(body, Terminator::Goto(head));

    b.push(exit, Node::Assume { reg: cond, polarity: false });
...
fn try_catch_finally_edges() {
    ...
    let r0 = b.new_reg();
    b.push(body, Node::Eval { reg: r0, expr: ExprRef { span: span(0, 5), ty: Type::Int } });
    b.set_terminator(body, Terminator::Goto(fin));
    b.add_edge(body, handler, EdgeKind::Exception { ty: Some(Type::String) });

    let r1 = b.new_reg();
    b.push(handler, Node::Eval { reg: r1, expr: ExprRef { span: span(6, 9), ty: Type::Int } });
...
fn field_place_path() {
    ...
    b.push(entry, Node::Assign { lhs: place, rhs: r0, span: span(2, 9) });
