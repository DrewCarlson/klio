...
//! of the API surface is produced by the `ktc-stdlib-gen` binary from the
...
pub fn param_names(fqn: &str) -> Option<&'static [&'static str]> {
    // Hand-impl table wins. Its FQNs match what `ktc-interp`'s dispatch
