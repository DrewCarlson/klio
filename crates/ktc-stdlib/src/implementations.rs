...
use ktc_runtime::{CallCtx, RuntimeError, StdlibFn, Value};
...
/// used in `TABLE`. Consulted by `ktc_stdlib::param_names` so callers can
...
fn float_to_string(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    ...
    Ok(Value::String(Rc::new(ktc_runtime::kotlin_float_to_string(d))))
}
...
fn double_to_string(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    ...
    Ok(Value::String(Rc::new(ktc_runtime::kotlin_double_to_string(d))))
}
...
fn coll_list_indices(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    ...
    Ok(Value::Range { start: 0, end: len - 1, step: 1, kind: ktc_runtime::RangeKind::Int })
}
...
fn make_sequence(items: Vec<Value>) -> Value {
    Value::Sequence(Rc::new(ktc_runtime::SequenceData {
        source: ktc_runtime::SequenceSource::Items(Rc::new(items)),
        ops: Vec::new(),
    }))
}
...
/// generator sources) goes through `ktc-interp`'s lazy materialize path.
fn recv_seq_eager(args: &[Value], what: &str) -> Result<Option<Rc<Vec<Value>>>, RuntimeError> {
    ...
    match &data.source {
        ktc_runtime::SequenceSource::Items(items) => Ok(Some(Rc::clone(items))),
        _ => Ok(None),
    }
}
...
fn ranges_down_to(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    ...
    Ok(Value::Range { start: a, end: b, step: -1, kind: ktc_runtime::RangeKind::Int })
}
...
fn ranges_until(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    ...
    Ok(Value::Range {
        ...
        step: 1,
        kind: ktc_runtime::RangeKind::Int,
    })
}
...
fn range_endpoint(kind: ktc_runtime::RangeKind, v: i64) -> Value {
    match kind {
        ktc_runtime::RangeKind::Long => Value::Long(v),
        ktc_runtime::RangeKind::Int => Value::Int(v as i32),
    }
}
...
fn range_reversed(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    ...
    Ok(Value::Range { start: *end, end: *start, step: -*step, kind: ktc_runtime::RangeKind::Int })
}
...
fn range_sum(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    ...
    Ok(match kind {
        ktc_runtime::RangeKind::Long => Value::Long(s),
        ktc_runtime::RangeKind::Int => Value::new_int(s),
    })
}
...
use ktc_runtime::{MatchData, MatchGroupData, RegexData};
...
fn match_result_range(ctx: &mut CallCtx) -> Result<Value, RuntimeError> {
    ...
    Ok(Value::Range { start: g0.start, end: g0.end_inclusive, step: 1, kind: ktc_runtime::RangeKind::Int })
}
...
mod tests {
    ...
    fn call(fn_: StdlibFn, args: &[Value]) -> Result<Value, RuntimeError> {
        let mut out = ktc_runtime::CaptureOutput::default();
    ...
    fn int_coerce_in_range_and_pair() {
        ...
        assert!(matches!(call(int_coerce_in, &[Value::Int(2), Value::Range { start: 0, end: 5, step: 1, kind: ktc_runtime::RangeKind::Int }]), Ok(Value::Int(2))));
    }
