...
//! with `kotlinc-native` and run under `ktc`, asserting byte-identical
...
fn check_paths(paths: &[PathBuf]) {
    if ktc_parity::find_kotlinc().is_err() {
        eprintln!(
            "kotlinc-native not found; skipping parity tests (install Kotlin Native at version {} \
             or set KTC_KOTLINC_NATIVE).",
            ktc_parity::TARGET_VERSION
        );
        return;
    }
    ...
    for p in paths {
        match ktc_parity::check(p) {
            ...
            Ok(report) => {
                ...
                eprint!("{}", ktc_parity::render_diff(&report));
                failed.push(p.clone());
            }
            Err(e) => {
