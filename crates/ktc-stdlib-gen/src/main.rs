...
//!                  `crates/ktc-stdlib/src/generated/`.
...
use ktc_stdlib_gen::{collect_decls, emit_generated};

#[derive(Parser)]
#[command(name = "ktc-stdlib-gen", about = "Kotlin stdlib codegen for ktc-stdlib")]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
}
...
enum Cmd {
    Build {
        ...
        /// (`<repo>/crates/ktc-stdlib/src/generated`).
        #[arg(long)]
        out: Option<PathBuf>,
    },
    Coverage,
...
fn workspace_root() -> PathBuf {
    // CARGO_MANIFEST_DIR is `crates/ktc-stdlib-gen` at build time; back off two
...
fn build(stdlib: Option<PathBuf>, out: Option<PathBuf>) -> ExitCode {
    ...
    let out_dir = out.unwrap_or_else(|| root.join("crates/ktc-stdlib/src/generated"));
...
fn coverage() -> ExitCode {
    ...
    if c.total == 0 {
        println!("registry is empty — run `ktc-stdlib-gen build` first");
    }
    ExitCode::SUCCESS
}
