...
pub struct Decl {
    ...
    /// Modifier bitset (mirrors `ktc_stdlib::Modifiers`).
    pub modifiers: u32,
...
// Modifier flag bits (must match `ktc_stdlib::Modifiers`).
pub mod modflag {
...
fn tokenize(src: &str) -> Vec<PosTok> {
    ...
    while i < bytes.len() {
        let b = bytes[i];
        if b == b'\n' {
            out.push(PosTok { tok: Tok::Newline, line, col });
            line += 1;
            col = 1;
            i += 1;
            continue;
        ...
        if b == b'@' {
            ...
            line = lines;
            col = end_col;
            out.push(PosTok { tok: Tok::Annotation, line, col });
            continue;
        }
        if is_ident_start(b) {
            let start = i;
            ...
            out.push(PosTok { tok: Tok::Ident(text), line, col: col - (i - start) as u32 });
            continue;
        }
        if b == b'`' {
            let start = i;
            ...
            let text = src[start..i].to_string();
            out.push(PosTok { tok: Tok::Ident(text), line, col });
            continue;
        }
        if b.is_ascii_digit() {
            let start = i;
            ...
            out.push(PosTok { tok: Tok::Number(src[start..i].to_string()), line, col });
            continue;
        }
        if b == b'(' {
            ...
            line = lines;
            col = end_col;
            out.push(PosTok { tok: Tok::Paren(inner), line, col });
            continue;
        }
        if b == b'[' {
            let (inner, end, lines, end_col) = balanced(bytes, i, b'[', b']', line, col);
            ...
            line = lines;
            col = end_col;
            out.push(PosTok { tok: Tok::Bracket(inner), line, col });
            continue;
        }
        if b == b'<' {
            ...
            if let Some((inner, end, end_line, end_col)) =
                try_balance_angles(bytes, i, line, col)
            {
                i = end;
                line = end_line;
                col = end_col;
                out.push(PosTok { tok: Tok::Angle(inner), line, col });
                continue;
            }
            out.push(PosTok { tok: Tok::Op('<'), line, col });
            i += 1;
            col += 1;
            continue;
        }
        ...
        if i + 1 < bytes.len() {
            ...
            if matches!(two, "->" | "::" | "..") {
                ...
                out.push(PosTok { tok: Tok::OpStr(tag), line, col });
                i += 2;
                col += 2;
                continue;
            }
        }
        out.push(PosTok { tok: Tok::Op(b as char), line, col });
        i += 1;
        col += 1;
    }
    out
...
fn consume_annotation(bytes: &[u8], mut i: usize, mut line: u32, mut col: u32) -> (usize, u32, u32) {
    // Eat `@`
    i += 1;
    col += 1;
...
fn balanced(bytes: &[u8], start: usize, open: u8, close: u8, line0: u32, col0: u32) -> (String, usize, u32, u32) {
...
fn try_balance_angles(bytes: &[u8], start: usize, line0: u32, col0: u32) -> Option<(String, usize, u32, u32)> {
    let mut depth = 0i32;
    let mut paren = 0i32;
    let mut i = start;
    let mut line = line0;
...
pub fn parse_file(src: &str) -> ParsedFile {
    ...
    let mut p = Parser { toks: &toks, pos: 0, package: String::new(), decls: Vec::new() };
    p.parse_top();
    ParsedFile { package: p.package, decls: p.decls }
}
...
impl<'a> Parser<'a> {
    ...
    fn read_dotted_name(&mut self) -> String {
        ...
        loop {
            // skip newlines and annotations? No, names are on one line generally.
            match self.peek() {
                Some(t) => match &t.tok {
                    Tok::Ident(s) => {
    ...
    fn parse_fun(&mut self, modifiers: u32, line: u32, col: u32, parent: Option<&str>) -> bool {
        ...
        let fqn = match parent {
            Some(p) => format!(
                "{}{}{}",
                if self.package.is_empty() { String::new() } else { format!("{}.", self.package) },
                format!("{}.", p),
                name
            ),
            None => format!(
    ...
    fn parse_class_like(
        ...
        let signature = format!("{} {}", match kind {
    ...
    fn parse_typealias(&mut self, modifiers: u32, line: u32, col: u32, parent: Option<&str>) -> bool {
        self.skip_newlines();
        let name = match self.peek() {
            Some(t) => match &t.tok {
                Tok::Ident(s) => {
...
mod tests {
    ...
    fn parses_property_in_class() {
        ...
        let pf = parse_file(src);
        let names: Vec<_> = pf.decls.iter().map(|d| (d.name.as_str(), d.parent.as_deref())).collect();
