...
impl Resolver {
    ...
    fn declare(
        ...
        if let Some(prev_id) = existing {
            ...
            if matches!(prev_kind, ScopeKind::File) {
                ...
                .with_code("R0004")
                .with_factory(&ktc_diagnostics::generated::factories::REDECLARATION);
    ...
    fn resolve_accessor(&mut self, parent: ScopeId, acc: &ktc_ast::Accessor) {
    ...
    fn resolve_expr(&mut self, scope: ScopeId, expr: &Expr) {
        match expr {
                ...
                for b in branches {
                    for p in &b.patterns {
                        match &p.kind {
                            ...
                            ktc_ast::WhenPatternKind::IsType(_)
                            | ktc_ast::WhenPatternKind::NotIsType(_)
            ...
            Expr::AnonFun { params, body, .. } => {
                ...
                match body.as_deref() {
                    Some(ktc_ast::FunctionBody::Block(b)) => {
                        self.resolve_block(fn_scope, b, /*new_scope=*/ false)
                    }
                    Some(ktc_ast::FunctionBody::Expr(e)) => self.resolve_expr(fn_scope, e),
                    None => {}
                }
            }
            Expr::PropertyRef { .. } => {
        ...
        if matches!(nullable, Some(false)) {
            self.diagnostics.emit(
                Diagnostic::from_factory(
                    &ktc_diagnostics::generated::factories::UNNECESSARY_SAFE_CALL,
                    op_span,
                )
                .with_message(format!(
        ...
        if let Some(sym) = self.lookup(scope, name) {
            ...
            self.diagnostics.emit(
                Diagnostic::error(format!("Unresolved reference `{name}`"), span)
                    .with_code("R0001")
                    .with_factory(&ktc_diagnostics::generated::factories::UNRESOLVED_REFERENCE),
            );
        }
    }
