...
| `io.ktor.client` | ~60% | No server module; HttpStatusCode/Headers types still thin | Typed JSON works end-to-end: `HttpClient { install(ContentNegotiation) { json() } }`, `body<T>()` response decode and `setBody<T>()` request encode round-trip through kotlinx.serialization (smoke-tested against a live server); builder DSL (`get(url){ header(...) }`) works |
