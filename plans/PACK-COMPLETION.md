...
| `kotlinx.serialization` | ~30% | No `builtins`, no `modules`, no `json` format; descriptor DSL broken | Reflective round-trip over `@Serializable` data classes works; collection/map serializers, JSON, polymorphism, nullable/enum descriptors, `SerializersModule` all absent |
