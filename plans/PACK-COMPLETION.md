...
| `io.ktor` | ~60% | HttpStatusCode/Headers types still thin; no plugins beyond ContentNegotiation | Client and server both work end-to-end with serialization-backed JSON. Client: `HttpClient { install(ContentNegotiation) { json() } }`, `body<T>()` / `setBody<T>()`, builder DSL. Server: `embeddedServer(CIO, port) { routing { get/post { call.respond/receive<T>() } } }` over a native blocking accept loop. Both smoke-tested against a live socket |
