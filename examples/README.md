...
| `compose_ui.kt`            | Compose UI core (`klio.compose.ui`) on a headless software canvas: a `Column`/`Row`/`Box`/`Text` tree emits `LayoutNode`s; a measure pass lays them out (arrangement + padding + size) and a draw pass paints backgrounds + 3×5 bitmap-font text into a software pixel buffer, dumped as ASCII. A state write recomposes and re-renders (the counter text updates) — the LayoutNode + measure/layout/draw path a real Compose UI uses, minus Skia. |
