...
| `compose_ui.kt`            | Compose UI core (`klio.compose.ui`) on a headless software canvas: a `Column`/`Row`/`Box`/`Text` tree emits `LayoutNode`s; a measure pass lays them out (arrangement + padding + size) and a draw pass paints backgrounds, `border` outlines, and 3×5 bitmap-font text into a software pixel buffer, dumped as ASCII. A state write recomposes and re-renders (the counter text updates) — the LayoutNode + measure/layout/draw path a real Compose UI uses, minus Skia. |
| `compose_ui_click.kt`      | The full interactive Compose UI loop on the software canvas: a clickable `Button` + counter `Text`; a simulated pointer click hit-tests the button, invokes its `onClick` (writing state), recomposes, and re-renders (`N 0` → `N 1` → `N 2`) — input → state → recompose → draw, driven deterministically. |
...
| `compose_ui_ppm.kt`        | The native rendering sink: the UI is rasterized to the software pixel buffer, then a native host binding (`src/compose_ui`) encodes it into a real P6 PPM image, writes it to disk, and returns a checksum of the encoded bytes — the headless "dumps a pixel buffer" backend (a native Skia binding would slot in the same seam). |
