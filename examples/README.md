...
| `select_and_semaphore.kt`  | `kotlinx.coroutines.selects.select` over channel `onReceive`/`onSend` + deferred `onAwait` + `onTimeout` clauses, and a `Semaphore` limiting concurrent permits with `withPermit` under contention. |
