...
| `compose_ui.kt`            | Compose UI core (`klio.compose.ui`) on a headless software canvas: a `Row`/`Column`/`Box` tree of sized, colored boxes emits `LayoutNode`s; a measure pass lays them out (arrangement + padding + size) and a draw pass paints them into a software pixel buffer, dumped as ASCII. A state write recomposes and re-renders (a box's colour flips) — the LayoutNode + measure/layout/draw path a real Compose UI uses, minus Skia. |
