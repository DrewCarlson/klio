...
| `compose_brush.kt`         | The real `graphics.Brush` — `SolidColor` applied to a `Paint` sets that flat colour; a gradient brush builds from its stops (painting gradients needs the deferred Skia shaders). |
