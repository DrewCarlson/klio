...
| `KLIO_FRAME_COUNT` / `KLIO_FRAME_CENSUS` | set | How many interpreted activations a workload runs (`activations` = register-bank acquisitions, one per real frame; `entries` = `runFrameExec` entries, higher because a flat call re-enters its caller's frame), and with `_CENSUS` the top functions by activation count. The frames-per-unit metric that separates "too many frames" from "frames too expensive" | `[frames]` |
