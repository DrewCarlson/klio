...
| `KLIO_FJ_SELF_INLINE` | `0` disables | The whole-function tier splices a self call (`this.helper(...)`) whose callee is a single-block scalar expression that never touches `this`. On by default: the receiver Move such a call needs is otherwise what makes the whole method uncompilable | `[jit] inlining self-call` |
