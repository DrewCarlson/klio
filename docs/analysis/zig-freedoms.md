...
#### 2C.5 Resolver/Resolution duplicate the "arena + gpa containers + manual deinit" pattern redundantly — RESOLVED (R20)

The resolver now allocates every container and string from the single
driver-owned arena it is handed; the child `*ArenaAllocator`, the
`Resolution.deinit`/`Scope.deinit` bodies, and the `arena`/`allocator` result
fields are deleted. Same shape applied to the type checker (`TypeCheck`/
`Checker`/`ClassInfo` deinits gone). Original finding kept for reference:
...
#### 2C.6 VM construction does a 14-field "move and leave empty" dance to dodge double-deinit — OUT OF R20 SCOPE (runtime layer, R16-owned)

This `vmFromBuilt` dance lives in the `interp_ir` VM/`ProgramImage` layer, not
a compiler phase; it is tied to the per-cell `ObjRef` allocator and the R16
`reclaim=false` gate (which already makes the `BuiltModule`/`Vm` teardown a
no-op on arena-backed runs). R20 covered the compiler phases (resolver/typeck)
only. Original finding kept for reference:
...
| R20 | **DONE.** Single driver-owned arena per compiler phase. **Ownership contract:** the driver hands the resolver and the type checker **one** allocator (production: the process-lifetime arena in `main.zig`; tests: an `ArenaAllocator(testing.allocator)`); each phase allocates **all** its data — scratch *and* result — from it and frees nothing, and the driver reclaims the whole phase workspace by freeing that one arena once the phase's last reader (resolution → typeck → lowering → image) is done. **Deleted deinit bodies (5):** `Resolution.deinit` + `Scope.deinit` (`src/resolver/resolver.zig`; also dropped the resolver's child `*ArenaAllocator` field, its `init` allocation, and the `arena`/`allocator` result fields — the per-file string arena now *is* the driver arena), `TypeCheck.deinit`, `Checker.deinit`, `ClassInfo.deinit` (`src/typeck/check.zig`), plus the two orphaned helpers `freeCfg` + `deinitMapOfList` that only the two deleted typeck deinits called. **Move dance eliminated:** the resolver no longer threads `fn_sig_keys` through a `defer …deinit()` scratch-free, and the typeck driver no longer hand-frees the `Checker`'s ~30 scratch containers; both phases just return the arena-backed result view. **Kept (with reason — NOT arena-owned phase teardown):** `Frame.deinit` (called mid-phase by `popFrame` on every scope exit — bounded intra-phase scope memory, not phase result); `ConstraintSystem.deinit` (releases a per-root-call **sub-arena** off the driver arena, freed by `expr_calls.zig` after each inference session — caps inference workspace mid-phase; making it driver-arena-backed would accumulate unboundedly per call); the value-level `Type`/`GenericArg`/`TypeError.deinit` (used pervasively as `defer t.deinit(a)` on **mid-phase scratch Types** across typeck — these free intra-phase temporaries, not the phase container, so their backing does not move to the arena). **2C.6's `vmFromBuilt` 14-field move dance is OUT of scope** — it lives in the `interp_ir` VM/`ProgramImage` layer tied to the R16 reclaim gate + per-cell `ObjRef` allocator, not a compiler phase; R16 already neutralizes that teardown under `reclaim=false`. **2A.6's per-cell `ControlBlock` allocator is R16-owned and untouched.** Consumers updated to drop the now-deleted calls: `commands.zig` (`runCheck`), `pack_build.zig` (`buildTypeckBundle`), `bench.zig` (resolve/typeck stages), and the resolver's 16 unit tests now wrap an `ArenaAllocator(testing.allocator)`; the typeck `checkFile`/`Checked` harness and `itests/typeck_negative.zig` already used the arena model. Net **−95 LOC** src. **Leak-checking proof:** all phase modules stay green on the leak-checking `testing.allocator` via the per-test arena — a phase arena freed too early would surface as a `DebugAllocator` UAF and too late as a leak; neither occurs. Gates: `zig build test --summary all` 139/139 steps · 1521/1521 tests · 0 skips; `corpus_check.py --no-rust` 83/83; differential 91 programs (11 pack-using ≥2 modes) byte-identical across 3 pack modes with per-program arena reset; suspend fuzzer `KLIO_FUZZ_SEEDS=128` 0 crashes / 0 cross-mode divergence; real-thread stress (`runtime_objref_threads`/`tl_wakeup_hammer`/`parity_threaded_litmus`) green under `KLIO_RACE_JITTER=1`; `KLIO_TRACE_INVARIANTS=1`/`KLIO_RESOLVE_AUDIT=1`/`KLIO_LINK_AUDIT=1` = 0; `zigcheck.py` resolver/typeck/types/interp_ir/ir each compile + pass standalone. 2C.5 retired; 2C.6 (compiler-phase half) and 2A.6 remain R16-owned/out-of-scope as noted. This was the last open zig-freedoms item. | high | high | XL | R16 |
...
path)** → **R20 (done — each compiler phase now allocates everything from one
driver-owned arena and exposes no container teardown; `Resolution.deinit`,
`Scope.deinit`, `TypeCheck.deinit`, `Checker.deinit`, `ClassInfo.deinit` and
